...
var ICONS = {
  ...
  daisy:
    'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 2.09 3 4.5S13.66 14 12 14s-3-2.59-3-4.5S10.34 5 12 5z',
...
const BADGES = [
  {
    ...
    emoji: '\uD83C\uDF31',
    label: 'Yeni Üye',
    desc: 'İlk gönderiyi paylaştı',
    check: function (s) {
      return s.postCount >= 1;
    },
  },
  {
    id: 'aktif_katilimci',
  ...
  },
  {
    ...
    emoji: '\u2B50',
    label: 'Yardımsever',
    desc: "5 'En İyi Cevap' aldı",
    check: function (s) {
      return s.bestAnswerCount >= 5;
    },
  },
  {
    id: 'not_paylasimci',
    emoji: '\uD83D\uDCDA',
    label: 'Not Paylaşımcısı',
    desc: '10 not paylaşımı yaptı',
    check: function (s) {
      return s.noteCount >= 10;
    },
  },
  {
    id: 'topluluk_lideri',
    ...
    desc: '100+ oy puanı kazandı',
    check: function (s) {
      return s.totalVoteScore >= 100;
    },
  },
  {
    id: 'ilk_adim',
    emoji: '\uD83D\uDC63',
    label: 'İlk Adım',
    desc: 'İlk yorumu yaptı',
    check: function (s) {
      return s.commentCount >= 1;
    },
  },
  {
    id: 'populer',
  ...
  },
  {
    ...
    desc: '20+ oy verdi',
    check: function (s) {
      return s.votesGiven >= 20;
    },
  },
  {
    id: 'usta',
    emoji: '\uD83C\uDF1F',
    label: 'Usta Paylaşımcı',
    desc: '25 gönderi paylaştı',
    check: function (s) {
      return s.postCount >= 25;
    },
  },
];
...
function calculateUserStats(userId, posts, allComments) {
  ...
  var noteCount = userPosts.filter(function (p) {
    return p.category === 'not';
  }).length;
  var questionCount = userPosts.filter(function (p) {
    return p.category === 'soru';
  }).length;
...
function validateFile(file, isInlineImage) {
  ...
  if (file.size > FILE_MAX_SIZE) {
    errors.push(
      "Dosya boyutu 5MB'dan büyük olamaz. Seçilen dosya: " +
  ...
  if (isInlineImage) {
    if (!file.type.startsWith('image/')) {
      errors.push(
        'Editöre sadece görsel dosyaları yüklenebilir. Seçilen dosya türü: ' +
          (file.type || 'bilinmiyor')
      );
    }
  } else {
    if (!ALLOWED_FILE_TYPES[file.type]) {
      var allowedExts = Object.values(ALLOWED_FILE_TYPES)
        .map(function (t) {
          return t.ext;
        })
        .join(', ');
      ...
      var observer = new IntersectionObserver(
        function (entries) {
          if (entries[0].isIntersecting && !_viewedPosts[postId]) {
            ...
            PortalDB.incrementViews(postId).catch(function (e) {
...
var PortalDB = {
  ...
  async deleteComment(postId, commentId) {
    ...
    var allComments = await window.apiRead('portal_posts_comments', {
      where: 'parentId:eq:s:' + String(commentId),
    });
    var deleteCount = 1;
    var ops = [];
...
function getCategoryInfo(catId) {
  return (
    PORTAL_CATEGORIES.find(function (c) {
      return c.id === catId;
    }) || PORTAL_CATEGORIES[0]
  );
}
...
function getReactionTotal(reactions) {
  ...
  var total = 0;
  Object.values(reactions).forEach(function (arr) {
    total += (arr || []).length;
  });
  return total;
}
      ...
      <button
        ...
        onMouseLeave={function (e) {
          if (!userUpvoted) {
            ...
            e.currentTarget.style.color = PC.textMuted;
          }
        }}
      >
        <SvgIcon
    ...
    >
      <button
        ...
        onMouseEnter={function (e) {
          ...
          e.currentTarget.style.borderColor = DY.gold;
        }}
        onMouseLeave={function (e) {
          ...
          {REACTION_TYPES.map(function (r) {
            ...
            return (
              <button
                ...
                style={{
                  padding: '6px 8px',
                  border: isActive ? '2px solid ' + DY.gold : '1px solid transparent',
                  borderRadius: 10,
                  background: isActive ? DY.goldLight : 'transparent',
                  cursor: 'pointer',
                  fontSize: 22,
                  display: 'flex',
                  flexDirection: 'column',
                ...
                onMouseLeave={function (e) {
                  ...
                  e.currentTarget.style.background = isActive ? DY.goldLight : 'transparent';
                }}
              >
                {r.emoji}
                {count > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 700, color: DY.warm }}>{count}</span>
                )}
              </button>
            );
          })}
      ...
      {reactions &&
        Object.keys(reactions).map(function (key) {
          ...
          var r = REACTION_TYPES.find(function (rt) {
            return rt.id === key;
          });
          if (!r) return null;
          ...
          return (
            <button
              ...
              style={{
                padding: '4px 10px',
                border: isActive ? '1.5px solid ' + DY.gold : '1px solid ' + PC.border,
                borderRadius: 16,
            ...
            >
              {r.emoji}{' '}
              <span style={{ fontSize: 12, fontWeight: 700, color: isActive ? DY.warm : PC.navy }}>
                {arr.length}
              </span>
            </button>
          );
...
const PollWidget = ({ pollOptions, pollVotes, postId, userId, onVote }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
      {pollOptions.map(function (option, idx) {
        ...
        return (
          <button
            ...
            style={{
              padding: '12px 16px',
              border: isSelected ? '2px solid ' + PC.blue : '1px solid ' + PC.border,
              borderRadius: 10,
              background: 'white',
              cursor: 'pointer',
              textAlign: 'left',
            ...
            onMouseEnter={function (e) {
              if (!hasVoted) e.currentTarget.style.borderColor = PC.blue;
            }}
            onMouseLeave={function (e) {
              if (!isSelected) e.currentTarget.style.borderColor = PC.border;
            }}
          >
            {/* İlerleme çubuğu */}
            {hasVoted && (
              <div
                style={{
                  ...
                  bottom: 0,
                  width: pct + '%',
                  background: isSelected ? PC.blueLight : PC.bg,
            ...
            >
              <span style={{ fontSize: 14, fontWeight: isSelected ? 700 : 500, color: PC.navy }}>
                {option}
              </span>
              {hasVoted && (
                <span
                  style={{
                    fontSize: 13,
                    fontWeight: 700,
                    color: isSelected ? PC.blue : PC.textMuted,
                  }}
                >
      ...
      <div style={{ fontSize: 12, color: PC.textMuted, marginTop: 4 }}>Toplam {totalVotes} oy</div>
    </div>
  );
};
...
const CommentItem = ({
  ...
  var handleSave = async function () {
    ...
    try {
      ...
      onUpdate(
        comment.id,
        comment.contentFormat === 'html' ? editContent : editContent.trim(),
  ...
  var editIsEmpty =
    comment.contentFormat === 'html' ? isHtmlEmpty(editContent) : !editContent.trim();
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span style={{ fontWeight: 700, fontSize: 13, color: PC.navy }}>
              {comment.authorName}
            </span>
            <span style={{ fontSize: 11, color: PC.textMuted }}>{timeAgo(comment.createdAt)}</span>
            {comment.editedAt && (
              <span style={{ fontSize: 10, color: PC.textMuted, fontStyle: 'italic' }}>
                (düzenlendi)
              </span>
            )}
            {comment.parentId && (
            ...
            {isOwner && !editing && (
              <div style={{ marginLeft: 'auto', display: 'flex', gap: 2 }}>
                <button
                  ...
                  onMouseEnter={function (e) {
                    e.currentTarget.style.color = PC.blue;
                  }}
                  onMouseLeave={function (e) {
                    e.currentTarget.style.color = PC.textMuted;
                  }}
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    fontSize: 12,
                    color: isLiked ? '#EF4444' : PC.textMuted,
                    display: 'flex',
                    alignItems: 'center',
                    gap: 4,
                    borderRadius: 10,
                ...
                {currentDepth < 2 && (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      fontSize: 12,
                      color: showReplyInput ? DY.gold : PC.textMuted,
                      display: 'flex',
                      alignItems: 'center',
                      gap: 4,
                      borderRadius: 10,
                ...
                {isQuestionPost &&
                  (isPostAuthor || currentUser.role === 'admin') &&
                  !comment.parentId && (
                    <button
                      onClick={function () {
                        if (onMarkBestAnswer) onMarkBestAnswer(comment.id);
                      }}
                      style={{
                        padding: '2px 8px',
                        border: 'none',
                        ...
                        cursor: 'pointer',
                        fontSize: 12,
                        color: isBestAnswer ? '#10B981' : PC.textMuted,
                        display: 'flex',
                        alignItems: 'center',
                        gap: 4,
                        borderRadius: 10,
                    ...
                    >
                      <svg
                        ...
                        height="13"
                        viewBox="0 0 24 24"
                        fill={isBestAnswer ? 'currentColor' : 'none'}
                        stroke="currentColor"
                        strokeWidth="2"
                      >
...
const CommentSection = ({ postId, currentUser, post, allUsers }) => {
  ...
  const handleSubmit = async function () {
    ...
    try {
      ...
      setNewComment('');
      setNewCommentResetKey(function (k) {
        return k + 1;
      });
  ...
  var rootComments = comments.filter(function (c) {
    return !c.parentId;
  });
  var replyMap = {};
    ...
    <div style={{ marginTop: 8 }}>
      ...
      {open && (
        ...
        >
          ...
          {loading ? (
            <div style={{ textAlign: 'center', color: PC.textMuted, padding: 12 }}>
              Yükleniyor...
            </div>
          ) : (
            <>
              {comments.length === 0 && (
                <div
                  style={{ textAlign: 'center', color: PC.textMuted, padding: 12, fontSize: 13 }}
                >
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      background:
                        isHtmlEmpty(newComment) || sending
                          ? PC.border
                      ...
                      boxShadow: isHtmlEmpty(newComment)
                        ? 'none'
...
const PostCard = ({
  ...
  if (post.contentFormat === 'html') {
    ...
    displayContent =
      !expanded && isLong ? post.content.substring(0, TRUNCATE_LEN) + '...' : post.content;
  }
  ...
  var handleSaveEdit = async function () {
    ...
    try {
      var updateData = {
        ...
        content: post.contentFormat === 'html' ? editContent : editContent.trim(),
      };
    ...
    >
      ...
      {post.status === 'rejected' && (
        ...
        >
          ...
          <span>
            Reddedildi{post.reviewedBy ? ' — ' + post.reviewedBy + ' tarafından' : ''}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span
                  onClick={function () {
                    if (onViewProfile) onViewProfile(post.authorId, post.authorName);
                    else if (onFilterAuthor) onFilterAuthor(post.authorName);
                  }}
                ...
                {/* Moderatör rozeti */}
                {moderators &&
                  moderators.some(function (m) {
                    return m.userId === post.authorId;
                  }) && (
                    <span
                      title="Moderatör"
                      style={{
                ...
                {post.authorId !== userId && onFollowUser && (
                  <button
                    ...
                    style={{
                      ...
                      border:
                        '1px solid ' +
                        (followedUsers && followedUsers.indexOf(post.authorId) >= 0
                          ? '#10B981'
                          : PC.border),
                      background:
                        followedUsers && followedUsers.indexOf(post.authorId) >= 0
                ...
                {post.courseCode && (
                  <span
                    onClick={function () {
                      if (onFilterTag) onFilterTag(post.courseCode);
                    }}
                    style={{
                      fontSize: 10,
                      padding: '2px 8px',
                      borderRadius: 4,
                      background: DY.goldLight,
                      color: DY.goldDark,
                      fontWeight: 700,
                      display: 'inline-flex',
                      alignItems: 'center',
              ...
              >
                ...
                {getVoteScore(post) !== 0 && (
                  <span
                    style={{
                      color: getVoteScore(post) > 0 ? '#16A34A' : '#DC2626',
                      fontWeight: 600,
                    }}
                  >
                    {' · '}
                    {getVoteScore(post) > 0 ? '+' : ''}
                    {getVoteScore(post)} oy
                  </span>
                )}
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {canModerate && (
                <button
                  ...
                  onMouseLeave={function (e) {
                    e.currentTarget.style.color = post.pinned ? '#F59E0B' : PC.textMuted;
                  }}
                >
                  <svg
                    width="16"
          ...
          {editing ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  disabled={
                    saving ||
                    (post.contentFormat === 'html' ? isHtmlEmpty(editContent) : !editContent.trim())
                  }
                  style={{
                    padding: '6px 16px',
                    border: 'none',
            ...
            <>
              ...
              {post.editedAt && (
                <div
                  style={{ fontSize: 11, color: PC.textMuted, marginTop: 4, fontStyle: 'italic' }}
                >
                  (düzenlendi)
                </div>
              )}
          ...
          {post.attachment &&
            (() => {
              ...
              var isOffice =
                /\.(pptx?|docx?|xlsx?)$/.test(nm) ||
                ty.indexOf('presentationml') !== -1 ||
                ty.indexOf('ms-powerpoint') !== -1 ||
                ty.indexOf('wordprocessingml') !== -1 ||
                ty.indexOf('msword') !== -1 ||
                ty.indexOf('spreadsheetml') !== -1 ||
                ty.indexOf('ms-excel') !== -1;
              ...
              var isFirebase = /firebasestorage\.googleapis\.com|\.firebasestorage\.app/.test(
                att.url || ''
              );
                ...
                >
                  {isImg && !isFirebase ? (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 13, fontWeight: 600, color: PC.navy, marginTop: 8 }}>
                        {att.name}
                      </div>
                      <div
                        style={{ fontSize: 12, color: PC.textMuted, marginTop: 6, lineHeight: 1.5 }}
                      >
                        Bu dosya eski depolama alanında bulunduğu için önizlenemiyor. Yeni
                        paylaşılan dosyalar normal şekilde önizlenir.
                      </div>
                      <a
                        href={att.url}
                        target="_blank"
                    ...
                    >
                      ...
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: PC.navy }}>
                          {att.name}
                        </div>
                        <div style={{ fontSize: 11, color: PC.textMuted }}>
                          {att.size ? (att.size / 1024).toFixed(0) + ' KB' : 'Dosya'}
                        </div>
                      </div>
                    </a>
                  )}
          ...
          >
            ...
            <button
              ...
              onMouseEnter={function (e) {
                e.currentTarget.style.borderColor = PC.blue;
                e.currentTarget.style.color = PC.blue;
              }}
              onMouseLeave={function (e) {
                e.currentTarget.style.borderColor = PC.border;
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 20,
                background: isBookmarked ? '#FEF3C7' : 'white',
                cursor: 'pointer',
                fontSize: 13,
                display: 'flex',
                alignItems: 'center',
                gap: 6,
                color: isBookmarked ? '#F59E0B' : PC.textMuted,
                transition: 'all 0.2s',
              }}
              onMouseEnter={function (e) {
...
const NewPostForm = ({ currentUser, onPost, onClose, allUsers }) => {
  ...
  var courseSuggestions = useMemo(
    function () {
      if (!courseCode.trim()) return [];
      var q = courseCode.toUpperCase();
      return POPULAR_COURSES.filter(function (c) {
        return c.includes(q);
      }).slice(0, 5);
    },
    [courseCode]
  ...
  const handleSubmit = async function () {
    ...
    try {
      ...
      if (file) {
        setUploading(true);
        try {
          var uploaded = await PortalDB.uploadFile(file);
      ...
      if (category === 'anket') {
        post.pollOptions = pollOptions.filter(function (o) {
          return o.trim();
        });
        post.pollVotes = {};
    ...
    >
      ...
      <div style={{ position: 'relative' }}>
        <RichTextEditor
          ...
          placeholder={
            category === 'soru'
              ? 'Sorunuzu detaylı bir şekilde yazın... (@kullanıcı ile bahsetebilirsiniz)'
              : category === 'anket'
              ...
              {courseSuggestions.map(function (c) {
                return (
                  <div
                    ...
                    onMouseEnter={function (e) {
                      e.currentTarget.style.background = DY.goldLight;
                    }}
                    onMouseLeave={function (e) {
                      e.currentTarget.style.background = 'white';
                    }}
      ...
      >
        ...
        {file ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            >
              <SvgIcon path={ICONS.x} size={16} color="#EF4444" />
            </button>
          </div>
        ) : (
          <div>
          ...
          {pollOptions.map(function (opt, idx) {
            return (
              <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: PC.textMuted, width: 20 }}>
                  {idx + 1}.
                </span>
                <input
                  value={opt}
                ...
                {pollOptions.length > 2 && (
                  <button
                    onClick={function () {
                      setPollOptions(
                        pollOptions.filter(function (_, i) {
                          return i !== idx;
                        })
                      );
                    }}
      ...
      <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        ...
        >
          {uploading ? 'Yükleniyor...' : posting ? 'Gönderiliyor...' : 'Paylaş'}
        </button>
      </div>
    </div>
  );
        ...
        <div
          ...
          style={{
            ...
            background:
              ...
              lvlInfo.current.color +
              ', ' +
              (lvlInfo.next ? lvlInfo.next.color : lvlInfo.current.color) +
              ')',
            borderRadius: 4,
...
const UserProfilePage = ({
  ...
  var userPosts = posts.filter(function (p) {
    return p.authorId === targetUserId;
  });
  ...
  useEffect(
    function () {
      PortalDB.getProfile(targetUserId)
        .then(function (p) {
          if (p) {
            setProfile(p);
            setEditBio(p.bio || '');
  ...
  var tabPosts = useMemo(
    function () {
      if (activeTab === 'posts') return userPosts;
      if (activeTab === 'notes')
        return userPosts.filter(function (p) {
          return p.category === 'not';
        });
  ...
  return (
    <div className="daisy-card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div
        style={{
          background:
            'linear-gradient(135deg, ' + lvlInfo.current.color + '20, ' + DY.goldLight + ')',
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: PC.navy, margin: 0 }}>
                {targetUserName}
              </h2>
              <span
                ...
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  background: lvlInfo.current.color + '20',
                  color: lvlInfo.current.color,
                  border: '1px solid ' + lvlInfo.current.color + '40',
                }}
              >
                Lv.{lvlInfo.current.level} {lvlInfo.current.label}
              </span>
              {!isOwnProfile && onFollowUser && (
                <button
                  ...
                  style={{
                    ...
                    color: isFollowed ? '#059669' : DY.goldDark,
                    cursor: 'pointer',
                  }}
                >
            ...
            {profile && profile.bolum && (
              <div style={{ fontSize: 12, color: PC.textMuted, marginTop: 4 }}>
                {profile.bolum}
                {profile.sinif ? ' · ' + profile.sinif : ''}
              </div>
            )}
            {profile && profile.bio && (
              <div style={{ fontSize: 13, color: PC.text, marginTop: 6, lineHeight: 1.5 }}>
                {profile.bio}
              </div>
            )}
            <LevelBar xp={xp} />
      ...
      <div style={{ padding: '16px 24px' }}>
        ...
        >
          {[
            ...
            {
              v: stats.totalVoteScore,
              l: 'Oy Puanı',
              c: stats.totalVoteScore >= 0 ? '#16A34A' : '#DC2626',
            },
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            <SvgIcon path={ICONS.star} size={14} color={DY.gold} /> Rozetler ({badges.length}/
            {BADGES.length})
          </h4>
          {badges.length > 0 ? (
            <BadgeDisplay badges={badges} />
          ) : (
            <div style={{ fontSize: 12, color: PC.textMuted }}>
              Henüz rozet kazanılmadı. Paylaşarak ve yardım ederek rozetler kazanın!
            </div>
          )}
            ...
            {BADGES.filter(function (b) {
              ...
              return (
                <span
                  key={b.id}
                  title={b.desc}
                  className="badge-item badge-item-locked"
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: 3,
        ...
        {profile && profile.courses && profile.courses.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <h4 style={{ fontSize: 13, fontWeight: 700, color: DY.warm, marginBottom: 6 }}>
              Aldığı Dersler
            </h4>
        ...
        {isOwnProfile && (
          <div style={{ marginBottom: 16 }}>
            {editing ? (
              ...
              >
                <h4 style={{ fontSize: 13, fontWeight: 700, color: DY.warm, marginBottom: 10 }}>
                  Profili Düzenle
                </h4>
                <div style={{ marginBottom: 8 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: PC.textMuted }}>
                    Biyografi
                  </label>
                  <textarea
                    value={editBio}
                ...
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: PC.textMuted }}>
                    Aldığı Dersler (virgülle ayırın)
                  </label>
                  <input
                    value={editCourses}
                    onChange={function (e) {
                      setEditCourses(e.target.value);
                    }}
        ...
        <div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: isActive ? 700 : 500,
                    border: 'none',
                    background: isActive ? DY.goldLight : PC.bg,
                    color: isActive ? DY.warm : PC.textMuted,
                    cursor: 'pointer',
                  }}
                >
                  {tab.label}
          ...
          {tabPosts.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center', color: PC.textMuted, fontSize: 13 }}>
              Gönderi bulunamadı.
            </div>
          ) : (
              ...
              {tabPosts.slice(0, 10).map(function (p) {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <div
                        style={{ fontSize: 13, fontWeight: 600, color: PC.navy, lineHeight: 1.4 }}
                      >
                        {p.title ||
                          (p.contentFormat === 'html'
                            ? stripHtmlTags(p.content).substring(0, 60)
                            : (p.content || '').substring(0, 60))}
                      </div>
                      <div style={{ fontSize: 10, color: PC.textMuted, marginTop: 2 }}>
                        {timeAgo(p.createdAt)} · {getVoteScore(p)} oy ·{' '}
    ...
    function () {
      ...
      if (period === 'week') {
        ...
        filteredPosts = posts.filter(function (p) {
          var d =
            p.createdAt && p.createdAt._seconds
      ...
      if (category === 'notes') {
        ...
        users.sort(function (a, b) {
          return b.bestAnswers - a.bestAnswers;
        });
      } else {
        // Genel: XP bazlı
  ...
  return (
    ...
    >
          ...
          return (
            <button
              ...
              style={{
                ...
                fontWeight: isActive ? 700 : 500,
                border: 'none',
                background: isActive ? '#E0E7FF' : PC.bg,
                color: isActive ? '#4F46E5' : PC.textMuted,
                cursor: 'pointer',
              }}
            >
              {c.label}
      ...
      {leaderboard.length === 0 ? (
        <div style={{ padding: 16, textAlign: 'center', color: PC.textMuted, fontSize: 12 }}>
          Bu dönemde veri yok.
        </div>
      ) : (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: PC.textMuted }}>
                  Lv.{userLvl.current.level} ·{' '}
                  {category === 'notes'
                    ? user.noteCount + ' not'
                    : category === 'questions'
                      ? user.bestAnswers + ' cevap'
                      : (user.score || 0) + ' puan'}
                </div>
              </div>
              <span
                style={{
                  ...
                  fontSize: 10,
                  fontWeight: 700,
                  background: userLvl.current.color + '20',
                  color: userLvl.current.color,
                }}
              >
                {userLvl.current.label}
              </span>
            </div>
          );
        })
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: PC.navy, lineHeight: 1.4 }}>
                  {p.title ||
                    (p.contentFormat === 'html'
                      ? stripHtmlTags(p.content).substring(0, 50)
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: '6px 0',
                  borderBottom:
                    idx < followData.users.length - 1 ? '1px solid ' + PC.borderLight : 'none',
                }}
              >
                <Avatar name={userName} size={24} />
                <span style={{ flex: 1, fontSize: 12, fontWeight: 600, color: PC.navy }}>
                  {userName}
                </span>
                <button
                  onClick={function () {
        ...
        {sortedTags.map(function (item) {
          ...
          return (
            <button
              key={item.tag}
              onClick={function () {
                onFilterTag(isActive ? '' : item.tag);
              }}
              style={{
                padding: '4px 10px',
                borderRadius: 12,
                fontSize: 10 + Math.round(intensity * 4),
                fontWeight: isActive ? 700 : 500,
                border: isActive
                  ? '2px solid ' + (isCourse ? DY.gold : '#6366F1')
                  : '1px solid ' + PC.border,
                background: isActive
                  ? isCourse
                    ? DY.goldLight
            ...
            >
              ...
              {onFollowTag && (
                <span
                  onClick={function (e) {
                    e.stopPropagation();
                    onFollowTag(item.tag);
                  }}
                  title={
                    followedTags && followedTags.indexOf(item.tag) >= 0
                      ? 'Takipten çık'
                      : 'Takip et'
                  }
                  style={{
  ...
  return (
    ...
    >
      {toasts.map(function (t) {
        ...
        var borderColor =
          t.type === 'error' ? '#EF4444' : t.type === 'info' ? '#3B82F6' : '#10B981';
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 10000,
        background:
          'linear-gradient(135deg, ' + (levelInfo.color || DY.gold) + ', ' + DY.goldDark + ')',
        color: 'white',
        padding: '16px 32px',
        borderRadius: 16,
...
const NotificationBell = ({ currentUser }) => {
  ...
  useEffect(
    function () {
      var load = function () {
        PortalDB.fetchNotifications(userId, 10)
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '10px 16px',
                    borderBottom: '1px solid ' + PC.borderLight,
                    background: n.read ? 'transparent' : DY.warmLight,
                  }}
                >
  ...
  return (
    <div className="daisy-card" style={{ padding: 20, marginBottom: 16 }}>
      <div
        ...
        onClick={function () {
          if (onViewProfile) onViewProfile(userId, currentUser.name);
        }}
      >
        <Avatar name={currentUser.name} />
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: DY.warm }}>
      ...
      >
        {[
          ...
          return (
            ...
            >
              <div style={{ fontSize: 16, fontWeight: 800, color: item.color || DY.warm }}>
                {item.v}
              </div>
      ...
      {onViewProfile && (
        <button
          onClick={function () {
            onViewProfile(userId, currentUser.name);
          }}
          style={{
            width: '100%',
            marginTop: 10,
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: 1 }}>
          <svg
            ...
            viewBox="0 0 24 24"
            fill="none"
            stroke={focused ? DY.gold : PC.textMuted}
            strokeWidth="2"
            style={{
              position: 'absolute',
              left: 14,
          ...
          <input
            ...
            style={{
              ...
              border: '1px solid ' + (focused ? DY.gold : PC.border),
              borderRadius: 12,
              fontSize: 14,
              outline: 'none',
              background: 'white',
        ...
        <button
          ...
          style={{
            padding: '10px 12px',
            border: '1px solid ' + (showAdvanced ? DY.gold : PC.border),
            borderRadius: 12,
            background: showAdvanced ? DY.goldLight : 'white',
            cursor: 'pointer',
            color: showAdvanced ? DY.goldDark : PC.textMuted,
            display: 'flex',
            alignItems: 'center',
            gap: 4,
            fontSize: 12,
            ...
            return (
              <div
                ...
                style={{
                  ...
                  gap: 10,
                  borderBottom:
                    idx < suggestions.length - 1 ? '1px solid ' + PC.borderLight : 'none',
                }}
                onMouseEnter={function (e) {
                  e.currentTarget.style.background = PC.bg;
                }}
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: PC.navy }}>
                    {highlightText(s.text, value)}
                  </div>
                  <div style={{ fontSize: 10, color: PC.textMuted }}>
                    {label}
                    {s.count ? ' (' + s.count + ' gönderi)' : ''}
                  </div>
                </div>
              </div>
            );
    ...
    >
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {pendingPosts.map(function (post) {
          var cat = getCategoryInfo(post.category);
          var textContent =
            post.contentFormat === 'html' ? stripHtmlTags(post.content) : post.content || '';
          var preview =
            textContent.length > 150 ? textContent.substring(0, 150) + '...' : textContent;
          var timeStr = post.createdAt
            ? timeAgo(
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontWeight: 600, fontSize: 14, color: PC.navy }}>
                  {post.authorName}
                </span>
                <span
                  style={{
                ...
                <span style={{ fontSize: 11, color: PC.textMuted, marginLeft: 'auto' }}>
                  {timeStr}
                </span>
              </div>
              {post.title && (
                <div style={{ fontWeight: 700, fontSize: 14, color: PC.navy, marginBottom: 4 }}>
                  {post.title}
                </div>
              )}
              <div
                style={{ fontSize: 13, color: PC.textSecondary, lineHeight: 1.5, marginBottom: 12 }}
              >
                {preview}
              </div>
...
const ModeratorPanel = ({ moderators, onAdd, onRemove }) => {
  ...
  var modIds = moderators.map(function (m) {
    return m.userId;
  });
  var filtered =
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: 12, opacity: 0.7 }}>
            Moderatörler gönderileri sabitleyebilir, düzenleyebilir ve silebilir
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: 20 }}>
        ...
        {moderators.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: PC.navy }}>
                          {mod.userName}
                        </div>
                        <div
                          style={{
          ...
          <div
            style={{ padding: '16px 0', textAlign: 'center', color: PC.textMuted, fontSize: 13 }}
          >
        ...
        <div>
          ...
          <input
            ...
            onFocus={function (e) {
              e.target.style.borderColor = PC.navy;
            }}
            onBlur={function (e) {
              e.target.style.borderColor = PC.border;
            }}
          />
...
function OgrenciPortaliApp({ currentUser }) {
  ...
  var showToast = function (message, type) {
    ...
    setToasts(function (prev) {
      return [].concat(prev, [{ id: id, message: message, type: type || 'success' }]);
    });
    setTimeout(function () {
      setToasts(function (prev) {
  ...
  var handleToggleBookmark = function (postId) {
    ...
    setBookmarks(function (prev) {
      ...
      var updated;
      if (idx >= 0) {
        updated = prev.filter(function (id) {
          return id !== postId;
        });
      } else {
  ...
  const handleNewPost = async function (postData) {
    ...
    if (isModOrAdmin) {
      ...
      showToast(
        'Gönderi incelenmek üzere gönderildi. Onaylandıktan sonra yayınlanacaktır.',
        'info'
      );
    }
    // Mention bildirimleri gönder
    ...
    mentions.forEach(function (mentionedName) {
      ...
      if (mentionedUser && mentionedUser.id !== userId) {
        PortalDB.addNotification(mentionedUser.id, {
          type: 'mention',
          message:
            (currentUser.name || 'Birisi') +
            ' sizi bir gönderide bahsetti: "' +
            (postData.title || 'Gönderi') +
            '"',
          postId: saved.id,
          fromUser: currentUser.name,
        });
    ...
    try {
      if (isModOrAdmin && window.StudentNotifier) {
        ...
        var excludeNumber =
          currentUser && currentUser.studentNumber ? currentUser.studentNumber : null;
        window.StudentNotifier.notifyDepartmentStudents(
          deptId,
          {
            type: 'portal_post',
            title: 'Öğrenci portalında yeni duyuru',
            message:
              (currentUser && currentUser.name ? currentUser.name : 'Bir kullanıcı') +
              ': "' +
      ...
      console.warn('Portal bildirimi gönderilemedi:', ne);
    }
  };
  ...
  const handleApprovePost = async function (postId) {
    try {
      ...
      setPosts(function (prev) {
        return prev.map(function (p) {
          return p.id === postId
            ? Object.assign({}, p, { status: 'approved', reviewedBy: currentUser.name })
            : p;
        });
      });
  ...
  var filteredPosts = useMemo(
    function () {
      ...
      if (authorFilter) {
        result = result.filter(function (p) {
          return p.authorName === authorFilter;
        });
      }
      // Etiket filtresi
      if (tagFilter) {
        ...
        result = result.filter(function (p) {
          var postTags = (p.tags || []).map(function (t) {
      ...
      if (dateRange.start) {
        ...
        result = result.filter(function (p) {
          ...
          var postDate = p.createdAt._seconds
            ? new Date(p.createdAt._seconds * 1000)
            : new Date(p.createdAt);
      ...
      if (sortMode === 'bookmarked') {
        result = result.filter(function (p) {
          return bookmarks.indexOf(p.id) >= 0;
        });
      }
      ...
      if (sortMode === 'following') {
        result = result.filter(function (p) {
          ...
          var followedTags = followData.tags.map(function (t) {
            return t.toLowerCase();
          });
  ...
  var pinnedPosts = filteredPosts.filter(function (p) {
    return p.pinned;
  });
  var regularPosts = filteredPosts.filter(function (p) {
    return !p.pinned;
  });
    ...
    <div ref={portalRootRef}>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 4px 40px' }}>
          ...
          <ModeratorPanel
            ...
            onAdd={function (user) {
              PortalDB.addModerator(user.id, user.name).then(function () {
                setModerators(function (prev) {
                  return prev.concat([{ id: user.id, userId: user.id, userName: user.name }]);
                });
        ...
        {highlightedPostId && (
          ...
          >
            ...
            >
              <SvgIcon path={ICONS.pin} size={16} color={DY.goldDark} /> Tek bir gönderi
              görüntüleniyor
            </span>
            <button
              onClick={function () {
          ...
          >
            <button
              ...
              style={{
                ...
                borderRadius: 20,
                fontSize: 13,
                fontWeight: activeCategory === 'tumu' ? 700 : 500,
                cursor: 'pointer',
                border:
                  activeCategory === 'tumu' ? '2px solid ' + PC.navy : '1px solid ' + PC.border,
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 16 : 20 }}>
            ...
            {!loading && filteredPosts.length === 0 && (
              ...
              >
                <div style={{ marginBottom: 16 }}>
                  <SvgIcon path={ICONS.file} size={48} color={DY.gold} />
                </div>
            ...
            {regularPosts.length > visibleCount && (
              <div
                ref={loadMoreRef}
                style={{ padding: 20, textAlign: 'center', color: PC.textMuted }}
              >
          ...
          {isMobile ? (
            <MobileSidebarToggle>
              ...
              <TagCloud
                posts={posts}
                onFilterTag={setTagFilter}
                activeTag={tagFilter.replace(/^#/, '')}
                onFollowTag={handleFollowTag}
                followedTags={followData.tags}
              />
                  ...
                  return (
                    <div key={cat.id} style={{ marginBottom: 10 }}>
                      ...
                      >
                        ...
                        >
                          <SvgIcon path={ICONS[cat.icon]} size={12} color={cat.color} /> {cat.label}
                        </span>
      ...
      {viewProfile && (
        <div
          ...
          onClick={function (e) {
            if (e.target === e.currentTarget) setViewProfile(null);
          }}
        >
          <div
            ...
            style={{
              ...
              maxHeight: '90vh',
              overflowY: 'auto',
              borderRadius: window.innerWidth <= 480 ? 12 : 20,
            }}
          >
            <UserProfilePage
