...
/**
 * Belgeden okunan satırdaki NOT KAYNAĞI: yüzlük puan mı, harf mi, hiçbiri mi?
 *
 * ── KARŞI KURUMUN HARFİ DOĞRUDAN TAŞINMAZ ──
 * Çeviri yüzlük puan üzerinden yapılır (bkz. lib/mezuniyet.js →
 * mezPuandanHarf). İki kurumun harf ölçeği aynı şeyi ifade etmez: birinde
 * BB 70-74 iken ötekinde 75-79 olabilir. Bu yüzden harf okunsa bile ÇAKÜ
 * karşılığı BURADA üretilmez — kararı bölüm verir.
 *
 * Harfi yine de okuyoruz: bazı transkriptlerde yüzlük puan hiç yok ve o
 * belge eskiden tümden reddediliyordu; öğrencinin elinde başka belge
 * olmadığı için başvuru orada kilitleniyordu.
 *
 * @returns 'puan' | 'harf' | 'yok'
 */
export function notKaynagi(satir) {
  const s = satir || {};
  if (String(s.not == null ? '' : s.not).trim()) return 'puan';
  if (String(s.harf == null ? '' : s.harf).trim()) return 'harf';
  return 'yok';
}

/** Harf notunu tek biçime getirir: büyük harf, boşluksuz. */
export function harfNormalize(harf) {
