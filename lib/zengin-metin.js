...
// Hizalama ve girinti taşıyabilen blok etiketler.
const HIZALANABILIR = new Set(['p', 'h3', 'h4', 'li', 'blockquote']);
...
// ── HİZALAMA ──
// Editör hizalamayı sınıf adıyla veriyor (Quill: `ql-align-center`). Sınıf
// adları olduğu gibi geçirilmiyor — sınıf, sayfanın kendi stil sayfasındaki
// herhangi bir kurala bağlanabilir. Bunun yerine TANINAN üç değer okunup
// ağaca `hiza` olarak yazılıyor; gösterim tarafı ondan `textAlign` üretiyor.
const IZINLI_HIZA = new Set(['center', 'right', 'justify']);
// Girinti düzeyi: yalnız sayı, en çok 4 kademe. Serbest bırakılsaydı
// `ql-indent-999` ile içerik ekrandan taşırılabilirdi.
const EN_COK_GIRINTI = 4;
...
function ozellikleriCoz(ad, ham) {
  ...
  // Blok etiketlerde hizalama ve girinti — sınıf adından OKUNUR, sınıf
  // olarak geçirilmez.
  if (HIZALANABILIR.has(ad)) {
    const sinif = zenginKacisCoz(oku('class')).toLowerCase();
    const h = /ql-align-([a-z]+)/.exec(sinif);
    if (h && IZINLI_HIZA.has(h[1])) out.hiza = h[1];
    const g = /ql-indent-(\d+)/.exec(sinif);
    if (g) {
      const k = parseInt(g[1], 10);
      if (k > 0) out.girinti = Math.min(k, EN_COK_GIRINTI);
    }
  }
  return out;
}
