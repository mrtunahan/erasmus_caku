...
export function satirXml(hucreler, satirNo, varsayilanStil, renkStilleri) {
  const icerik = (hucreler || [])
    .map((h, i) => {
      ...
      // SAYI hücresi: değer JS sayısı olarak geldiyse Excel'e sayı yazılır —
      // rapor sayfalarında ortalama sütunu toplanabilsin, sıralanabilsin.
      // Dize gelen '2' ise METİN kalır: "08:15-09:00" ya da "2. Sınıf" gibi
      // değerlerin tarihe/sayıya dönüşmemesi bu dosyanın eski kuralıdır.
      if (typeof nesne.v === 'number' && Number.isFinite(nesne.v)) {
        return '<c r="' + ref + '"' + s + '><v>' + nesne.v + '</v></c>';
      }
      return (
        '<c r="' +
        ref +
...
/**
 * Excel sayfa adı kuralı: en çok 31 karakter, birkaç işaret yasak ve AYNI ad
 * iki kez kullanılamaz — kullanılırsa Excel dosyayı "onarmak" ister ve
 * sayfalardan biri kaybolur. Çakışan ada sıra numarası eklenir.
 */
export function sayfaAdiDuzelt(ad, kullanilanlar) {
  const kume = kullanilanlar instanceof Set ? kullanilanlar : new Set();
  const temiz = String(ad == null || ad === '' ? 'Sayfa1' : ad)
    .replace(/[\\/*?:[\]]/g, ' ')
    .trim()
    .slice(0, 31);
  let sonuc = temiz || 'Sayfa1';
  let n = 2;
  while (kume.has(sonuc.toLocaleLowerCase('tr'))) {
    const ek = ' (' + n++ + ')';
    sonuc = (temiz || 'Sayfa1').slice(0, 31 - ek.length) + ek;
  }
  kume.add(sonuc.toLocaleLowerCase('tr'));
  return sonuc;
}

/** Seçeneği sayfa listesine çevirir — tek sayfalık eski çağrı da çalışsın. */
function sayfalariCoz(s) {
  if (Array.isArray(s.sayfalar) && s.sayfalar.length > 0) return s.sayfalar;
  return [
    {
      ad: s.sayfaAdi,
      satirlar: s.satirlar,
      ...
      govdeStili: s.govdeStili,
    },
  ];
}

/**
 * Bir çalışma kitabının TÜM parçaları (yol → içerik).
 *
 * Tek sayfa için `{ sayfaAdi, satirlar, … }`, çok sayfa için
 * `{ sayfalar: [{ ad, satirlar, … }] }` verilir. Çok sayfa, raporun ham
 * veriyle özeti aynı dosyada ama AYRI sekmelerde tutabilmesi içindir:
 * tek sayfaya yığılan rapor süzülemez de sıralanamaz da.
 *
...
export function calismaKitabiParcalari(secenek) {
  ...
  // Stil sayfası kitabın tamamında ortaktır — renkler tüm sayfalardan toplanır.
  const tumRenkler = [];
  sayfalar.forEach((sf) =>
    renkleriTopla(sf && sf.satirlar).forEach((a) => {
      if (!tumRenkler.includes(a)) tumRenkler.push(a);
    })
  );
  const katalog = stilKatalogu(tumRenkler);
  const kullanilan = new Set();
  const adlar = sayfalar.map((sf, i) =>
    sayfaAdiDuzelt((sf && sf.ad) || (i === 0 ? s.sayfaAdi : '') || 'Sayfa' + (i + 1), kullanilan)
  );
  const stilRId = 'rId' + (sayfalar.length + 1);
  ...
  const parcalar = {
    ...
    'xl/workbook.xml':
      ...
      adlar
        .map(
          (ad, i) =>
            '<sheet name="' + kacis(ad) + '" sheetId="' + (i + 1) + '" r:id="rId' + (i + 1) + '"/>'
        )
        .join('') +
      '</sheets></workbook>',
  ...
  sayfalar.forEach((sf, i) => {
    const o = sf || {};
    parcalar['xl/worksheets/sheet' + (i + 1) + '.xml'] = sayfaXml(o.satirlar, {
      sutunGenislikleri: o.sutunGenislikleri,
      baslikStili: XLSX_STIL.baslik,
      govdeStili: o.govdeStili != null ? o.govdeStili : XLSX_STIL.govde,
      baslikIndeksi: o.baslikIndeksi,
      dondurSatir: o.dondurSatir,
