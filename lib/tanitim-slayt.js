...
//
// ── KİMLİKLER İKİ YERDE, AMA KAYMAMALI ──
// Bu liste yönetim ekranının modül seçicisini besler. Tanıtım sayfası saf
// HTML olduğu ve buradan import edemediği için aynı kimlikleri kendi
// içinde de taşır. İkisi ayrışırsa yetkilinin "Staj" diye eklediği slayt
// ziyaretçide hiçbir sekmede görünmez — sessiz bir kayıp. Bu yüzden
// tests/tanitim-slayt.test.js public/tanitim.html'i okuyup iki listenin
// kimlik kümesini karşılaştırır.
// ══════════════════════════════════════════════════════════════
export const TANITIM_MODULLERI = [
...
const MODUL_IDLER = new Set(TANITIM_MODULLERI.map((m) => m.id));

/** Kayıttaki modül kimliği geçerli mi? */
export function modulGecerliMi(id) {
  return MODUL_IDLER.has(metin(id));
}
 ...
 * Yönetim ekranı bu kayıtları "modülsüz" diye işaretler.
 */
export function modulSlaytlari(kayitlar, modulId) {
  const hedef = metin(modulId);
  if (!hedef) return [];
