...
export function veriKapsami(girdi) {
  ...
  return {
    ...
    fakulteId: metin((p && p.fakulteId) || g.fakulteId),
    uni: !!g.uni,
    dept: !!g.dept,
    faculty: !!g.faculty,
  };
...
/**
 * Kapsam içindeki bölümler — ORG YAPISINDAN türetilir.
 *
 * ⚠ BÖLÜM LİSTESİ AKADEMİSYENLERDEN TÜRETİLEMEZ. Eskiden seçilebilir
 * bölümler `professors` kayıtlarından çıkarılıyordu: henüz akademisyen
 * kaydı girilmemiş bir bölüm (ör. İstatistik) listede HİÇ görünmüyordu.
 * Bölüm yetkilisi kendi bölümünü seçemiyor, fakülte yetkilisi de o bölümün
 * eksik olduğunu göremiyordu — "bölüm yok" ile "bölümde veri yok" birbirine
 * karışıyordu. Liste artık `departments` kayıtlarından gelir; akademisyeni
 * olmayan bölüm de listede durur ve "veri göndermedi" diye görünür.
 *
 * @param {Array} bolumKayitlari [{ id, ad, fakulteId, fakulteAdi }]
 */
export function kapsamBolumleri(bolumKayitlari, kapsam) {
  const liste = (Array.isArray(bolumKayitlari) ? bolumKayitlari : []).filter(
    (b) => b && (metin(b.id) || metin(b.ad))
  );
  const k = kapsam || {};
  let secili = [];
  if (k.uni) {
    ...
    // Fakülte eşleşmesi önce kimlikle, yoksa adla: kayıtların bir kısmında
    // yalnız biri dolu olabiliyor.
    if (!metin(k.fakulteId) && !metin(k.fakulte)) secili = [];
    else
      secili = liste.filter(
        (b) =>
          (metin(k.fakulteId) && metin(b.fakulteId) === metin(k.fakulteId)) ||
          (metin(k.fakulte) && anahtar(b.fakulteAdi) === anahtar(k.fakulte))
      );
  } else if (k.dept && metin(k.departmentId)) {
    secili = liste.filter((b) => metin(b.id) === metin(k.departmentId));
  }
  const gorulen = new Set();
  return secili
    .filter((b) => {
      const anah = metin(b.id) || anahtar(b.ad);
      if (gorulen.has(anah)) return false;
      gorulen.add(anah);
      return true;
    })
    .map((b) => ({ id: metin(b.id), ad: metin(b.ad) || metin(b.id) }))
 ...
 */
export function gonderimDurumu(bolumKayitlari, kapsam, gonderimler, yil) {
  // ⚠ Org yapısındaki TÜM bölümler: akademisyeni olmayan bölüm de
  // "göndermedi" diye görünmeli, listeden düşmemeli.
  const bolumler = kapsamBolumleri(bolumKayitlari, Object.assign({}, kapsam, { dept: false }));
  const gonderen = [];
  const bekleyen = [];
  bolumler.forEach((b) => {
