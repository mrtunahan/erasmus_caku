 ...
 * @param {object} [secenek]        { esikDisi:Set<string> } eşik altında kalanlar
...
export function asilYedekOner(kayitlar, turId, kontenjan, yedekSayisi, secenek) {
  // Taban puan şartını (kurum eşiği ya da programın kendi taban puanı)
  // karşılamayanlar sıralamaya girmez: şartı taşımıyorlar, kontenjanı
  // işgal etmemeliler ve YEDEK bile olamazlar.
  ...
  const elenmeNedeni = (id) =>
    typeof esikDisi.get === 'function' ? esikDisi.get(String(id)) || 'taban_osym' : 'taban_osym';
  ...
  gruplar.forEach((grup, sinif) => {
    ...
    sirali.forEach((k) => {
      ...
      // Taban ÖSYM puanının altındaki aday kontenjana SAYILMADAN elenir.
      if (elendiMi(id)) {
        oneri.push({
          id,
