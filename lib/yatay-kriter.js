...
/**
 * Bir başvurunun ELENME nedeni — iki ayrı taban sıralama şartı vardır:
 *
 *   1) `esik` → kurumun elle girdiği, tüm başvurulara uygulanan taban başarı
 *      sıralaması (ör. 300.000).
 *   2) `basvurduguBolumTabanSirasi` → adayın BAŞVURDUĞU PROGRAMIN kendi taban
 *      başarı sırası. Merkezi yerleştirmeyle geçişte aday, gitmek istediği
 *      programın taban sıralamasını da karşılamak zorundadır.
 *
 * Şartı karşılamayan kayıt yedek bile olamaz; sıralamaya girmeden elenir.
 *
 * @returns {''|'taban_sira'|'program_sira'}
 */
