...
// ── Personelin verdiği ELEYİCİ kararlar ──
//
// Değerlendirme listesindeki bazı sonuçlar sıralama sorunu değil, başvurunun
// KABUL EDİLEBİLİRLİĞİ sorunudur: eksik/hatalı belge, şartları taşımama,
// başvurudan vazgeçme. Bunlar sıralamaya hiç girmemeli.
//
// ⚠ Girmedikleri bilinmiyordu: personel "EKSİK BELGE" işaretlediği hâlde
// sıralama o kaydı ASİL önerip kontenjandan bir yer kapatıyordu; kartta
// "EKSİK BELGE" ile "UYGUN 3. Sınıf (1. ASİL)" yan yana duruyordu.
const MANUEL_ELEME = new Set(['eksik_belge', 'sartlari_tasimiyor', 'basvuru_geri']);

/** Personel bu kaydı eleyici bir sonuçla işaretlemiş mi? */
export function manuelElemeMi(degerlendirme) {
  return MANUEL_ELEME.has(String(degerlendirme || ''));
}
...
export function elemeNedeni(kayit, esik) {
  if (!kayit) return '';
  // Personelin açık kararı önce gelir: "eksik belge" diyen bir insanın
  // gerekçesini hesaplanmış bir gerekçeyle değiştirmek, kararı sahiplenmemek
  // olurdu. Aritmetik şartlar sıralanabilirliği, bu karar kabul
  // edilebilirliği söyler.
  if (manuelElemeMi(kayit.degerlendirme)) return String(kayit.degerlendirme);
...
export const ELEME_ETIKET = {
  ...
  eksik_belge: 'Eksik / hatalı belge',
  sartlari_tasimiyor: 'Başvuru şartlarını taşımıyor',
  basvuru_geri: 'Başvurudan vazgeçti',
...
export function gecerliDegerlendirme(kayit, esik) {
  ...
  return {
    ...
    degerlendirmeSira: '',
    sebep,
    // Personelin kendi eleyici kararı bir ÇAKIŞMA değildir; "önceki
    // sıralamadan kalan sonuç" uyarısı yalnız gerçekten artakalan
    // asil/yedek değerleri için anlamlı.
    cakisma:
      !!kayitli.degerlendirme &&
      kayitli.degerlendirme !== 'uygun_degil' &&
      !manuelElemeMi(kayitli.degerlendirme),
  };
}
