...
// ── Hedef (ÇAKÜ) harf notları ──
//
// Türk üniversiteleri iki yaygın ölçekten birini kullanıyor: AA/BA/BB/… ya da
// A1/A2/B1/B2/B3/C1/… Bu liste İKİSİNİ BİRDEN kapsıyor ve yalnız VARSAYILANDIR:
// kurumun kendi ölçeği farklıysa doğrulamaya dışarıdan liste verilebilir
// (bkz. notTablosuDogrula'nın ikinci parametresi; arayüz bunu bölümün
// mezuniyet kurallarındaki geçer/kalır listelerinden besliyor).
//
// ⚠ Bu liste önce yalnız AA/BA/… içeriyordu ve B1/C2/F1 kullanan bir kurumun
// tablosu "geçersiz harf" diye reddediliyor, akademisyen tabloyu ONAYLAYAMIYORDU.
// Varsayımı koda gömmek yerine yapılandırılabilir kılmanın sebebi bu.
...
// Geçer sayılan harfler — "bu dersi geçmiş mi" sorusu için. F ile başlayanlar
// ve K/DZ geçmez. DD/DC/D1 gibi sınır notları bazı yönetmeliklerde koşulludur;
// bölümün mezuniyet kuralındaki `gecerNotlar` listesi son sözü söyler.
 ...
 */
export function notTablosuDogrula(tablo, izinliHarfler) {
  const sorunlar = [];
  const satirlar = (tablo && Array.isArray(tablo.satirlar) && tablo.satirlar) || [];
  if (satirlar.length === 0) sorunlar.push('Tablo boş.');

  // Kurumun kendi harf ölçeği verilmişse O geçerlidir; verilmezse yaygın iki
  // ölçeği birden kapsayan varsayılan liste kullanılır.
  const kaynak =
    Array.isArray(izinliHarfler) && izinliHarfler.length > 0 ? izinliHarfler : CAKU_HARFLERI;
  const gecerliHarfler = new Set(kaynak.map(notNormalize).filter(Boolean));
  satirlar.forEach((r, i) => {
    ...
    if (!caku) sorunlar.push(no + 'ÇAKÜ notu boş.');
      ...
      sorunlar.push(
        no + '"' + r.caku + '" tanınan harf notları arasında değil (' + kaynak.join(', ') + ').'
      );
    }
