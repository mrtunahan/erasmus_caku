...
/**
 * Memurun ERİŞEBİLECEĞİ bölümler — bölüm şeridi bunu göstermeli.
 *
 * ⚠ BU KURAL BİR ÇELİŞKİDEN DOĞDU. Sistemde "bu memur hangi bölümleri
 * kapsıyor" sorusunun İKİ ayrı cevabı vardı ve bir vakada çeliştiler:
 *
 *   • Bölüm şeridi `memurBolumleri()` kullanıyordu — yalnız ATAMA KAYITLARI.
 *   • Belge görünürlüğü `memurBelgeModulleri()` kullanıyor — atama kaydı YOKSA
 *     memurun KENDİ bölümünde eski düz listeye (memurModules) düşer.
 *
 * Sonuç: ataması başka bölümlerde olan ama kendi bölümü Bilgisayar olan bir
 * memur, Bilgisayar'ı şeritte SEÇEMİYOR; görünürlük kuralı ise "seçili bölüm
 * belgenin bölümü olmalı" diyor. Belge sunucuda "görünür", ekranda asla
 * görünmüyordu — ve hiçbir uyarı da çıkmıyordu.
 *
 * Tek tanım: memurun erişebildiği bölüm, BELGE GÖREBİLDİĞİ bölümdür. Aday
 * bölümler (atama kayıtları + kendi bölümü) tek tek aynı kuraldan geçirilir,
 * böylece şerit ile görünürlük tanım gereği uyumlu kalır.
 */
export function memurErisebilecegiBolumler(atamalar, kullanici) {
  if (!memurMu(kullanici)) return [];
  const memurId = memurKimligi(kullanici);
  const ad = memurAdi(kullanici);
  const adaylar = [];
  const ekle = (b) => {
    const t = metin(b);
    if (t && !adaylar.includes(t)) adaylar.push(t);
  };
  // Kendi bölümü başta: varsayılan aktif bölüm odur.
  ekle(kullanici.departmentId);
  (atamalar || []).forEach((a) => {
    if (atamaBuMemurun(a, memurId, ad)) ekle(a.departmentId);
  });
  return adaylar.filter((b) => memurBelgeModulleri(atamalar, b, kullanici).length > 0);
}
