...
/**
 * Bu bölümdeki modüller NEREDEN geliyor?
 *
 * ⚠ BU AYRIM BİR YANILGIDAN DOĞDU. Memurlar ekranı, modülleri
 * `memurModulleri()` ile çözüyor — o da atama kaydı yoksa memur kaydındaki
 * ESKİ DÜZ LİSTEYE düşüyor. Ekran ikisini ayırmadığı için, bölüm için hiç
 * atama kaydı olmayan bir memurda bile "4 modül • bu bölümde atanmış"
 * yazıyordu. Yetkili atamanın yapıldığını sanıyor, kayıt ise yok.
 *
 * Fark görünmez değil, SONUÇ DOĞURUYOR: bölüm şeridi ve başka ekranlar atama
 * kayıtlarına bakıyor. Eski listeyle gelen erişim, o liste temizlendiği anda
 * sessizce kayboluyor.
 *
 *   'atama'      → bu bölüm için gerçek atama kaydı var (sağlam)
 *   'eski-liste' → kayıt yok; erişim memur kaydındaki eski listeden geliyor
 *   'yok'        → bu bölümde modülü yok
 */
export function memurAtamaKaynagi(atamalar, bolumId, memurId, eskiModuller) {
  const atama = memurAtamasi(atamalar, bolumId, memurId);
  if (atama) return dizi(atama.modules).length > 0 ? 'atama' : 'yok';
  return dizi(eskiModuller).length > 0 ? 'eski-liste' : 'yok';
}
