...
export function eslesmeNotlariniHesapla(donusEslesmeleri, transkript, sistem) {
  ...
  return (donusEslesmeleri || []).map((m) => {
    ...
    // `homeGrades` karşı kurum dersinin indeksiyle yazılır; oysa kendi
    // kurumumuzdaki karşılık ayrı bir listedir ve sayıları tutmayabilir
    // (iki karşı ders tek dersimize sayılabilir). O tek ders için geçerli
    // not, AKTS ağırlıklı ortalamadır — ilk dersin notu değil.
    const toplu = eslesmeHarfNotu(m, eslesen, sistem);
    return {
      id: m && m.id,
      hostGrades,
      homeGrades,
      homeGrade: toplu.ok ? toplu.harf : '',
      eksik,
    };
  });
}
...
// ══════════════════════════════════════════════════════════════
// BELGE SATIRININ NOTLARI
//
// Aynı öncelik üç ayrı yerde elle yazılıyordu (kart, HTML belge, şablon
// belgesi) ve üçü de aynı değildi: şablon yolu onaylanmış harf notunu hiç
// okumuyor, ham notu kendi başına yeniden çevirmeye çalışıyordu. Sözel
// ölçekli bir transkriptte bu boş dönüyor ve resmî belgenin "Başarı Notu"
// sütunu sessizce boş çıkıyordu.
//
// Kural tek yerde: ham not karşı kurum dersinindir (ders bazlı), harf notu
// eşleştirmenin BÜTÜNÜNE aittir — iki karşı ders tek dersimize sayıldığında
// geçerli olan AKTS ağırlıklı tek nottur, ilk dersin notu değil.
// ══════════════════════════════════════════════════════════════

/**
 * @param {object} eslesme dönüş eşleştirmesi
 * @param {number} i       karşı kurum dersinin indeksi
 * @returns {{ham: string, harf: string}}
 */
export function satirNotlari(eslesme, i) {
  const m = eslesme || {};
  const hg = m.hostGrades && typeof m.hostGrades === 'object' ? m.hostGrades[i] : undefined;
  const mg = m.homeGrades && typeof m.homeGrades === 'object' ? m.homeGrades[i] : undefined;
  const ham = hg || m.hostGrade || '';
  const harf = m.homeGrade || mg || '';
  return { ham: String(ham || ''), harf: String(harf || '') };
}
