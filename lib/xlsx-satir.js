...
/**
 * Sayfa XML'indeki <row> parçalarını SIRAYLA ayırır.
 *
 * Kendini kapatan satır ('<row r="5"/>' — Excel biçimli ama boş satırları
 * böyle yazar) ÖNCE denenir. Aksi hâlde açılış deseni ('<row' + [^>]* + '>')
 * onunla da eşleşiyor ve bir sonraki '</row>'a kadar her şeyi yutuyor;
 * ardındaki VERİ satırı görünmez olduğu için belge boş çıkıyordu.
 */
export const satirDeseni = () => /<row\b[^>]*\/>|<row\b[^>]*>[\s\S]*?<\/row>/g;

export function satirlariAyir(xml) {
  return [...String(xml || '').matchAll(satirDeseni())].map((m) => m[0]);
}

/**
 * Doldurulmuş satırları sayfa XML'ine geri yerleştirir.
 *
 * `uretilenler` şablondaki satır sayısı kadar ELEMAN taşır: veri satırının
 * yerine, kopyalarının BİRLEŞTİRİLMİŞ metni konur. Kopyalar ayrı eleman olarak
 * sıraya konursa değiştirme şablondaki satır sayısı kadar çağrıldığı için
 * sondaki satırlar (tarih, hazırlayan, imza) sessizce DÜŞER — bu fonksiyon o
 * sözleşmeyi tek yerde tutar.
 */
export function satirlariYerlestir(xml, uretilenler) {
  const liste = uretilenler || [];
  let i = 0;
  return String(xml || '').replace(satirDeseni(), () => liste[i++] || '');
}
