...
//   • kod + şube + hoca aynı → aynı ders, ikinci kez eklenemez
//   • kod aynı, şube farklı   → ayrı kayıt, eklenir
//   • kod aynı, HOCA farklı   → ayrı kayıt, eklenir (şube numarası gerekmez)
// Çıktıda kod "FZK181 (Şb:2)" diye yazılır; şubesiz ders eskisi gibi yalın.
//
// Hocanın ayırt edici olması "Bitirme Projesi" ve lisansüstü "Uzmanlık Alanı
// Dersi" içindir: tek kodla, aynı saatte, her danışmanın kendi grubu yürür.
//
// ── VERİ BİÇİMİ ──
//   slot = {
...
import { ayniEgitmen } from './ders-egitmenleri.js';
...
function dersNormalize(ham, birinci) {
  ...
  return {
    ...
    sube: metin(d.sube),
    // Çakışma onayı da devralınmaz: karar ders başınadır.
    ...(d.cakismaOnayi ? { cakismaOnayi: true } : {}),
  };
}
...
export function slotDersleri(slot) {
  ...
  const birinci = {
    ...
    sinif: s.sinif,
    sube: metin(s.sube),
    // Yerleştirirken çakışma uyarısı bilerek onaylandıysa iz burada durur.
    // Normalleştirme alanları TEK TEK seçtiği için taşınmayan alan kaybolur;
    // bu bayrak taşınmazsa "bilerek kabul edildi" ayrımı ilk okumada silinir.
    ...(s.cakismaOnayi ? { cakismaOnayi: true } : {}),
  };
...
/**
 * Bu ders (kod + ŞUBE + HOCA) slotta zaten var mı?
 *
 * Aynı kodun farklı şubesi AYRI derstir ve engellenmez — bir ders iki
 * müfredatta ve her müfredatta iki şubede okutulabiliyor.
 *
 * ── HOCA DA AYIRT EDİCİDİR ──
 * "Bitirme Projesi" ya da lisansüstü "Uzmanlık Alanı Dersi" aynı kodla, aynı
 * saatte, farklı hocalarla yürür: X hocanın grubu ile Y hocanın grubu ayrı
 * kayıttır. Yalnız koda bakan denetim ikincisini "zaten var" sayıp engelliyor
 * ya da gereksiz yere şube numarası istiyordu.
 *
 * `egitmen` verilmezse hoca yok sayılır (eski çağrılar aynen çalışır).
 */
export function slotDersVarMi(slot, dersKodu, sube, egitmen) {
  const k = metin(dersKodu);
  if (!k) return false;
  const sb = metin(sube);
  const hocayaBak = egitmen !== undefined;
  return slotDersleri(slot).some(
    (d) =>
      d.courseCode === k &&
      metin(d.sube) === sb &&
      (!hocayaBak || ayniEgitmen(d.instructor, egitmen))
  );
}

/**
 * Bu ders kodu slotta (herhangi bir şubeyle) geçiyor mu?
 * `egitmen` verilirse yalnız O HOCANIN kayıtları sayılır — farklı hocanın
 * aynı dersi şube numarası gerektirmez.
 */
export function slotKodVarMi(slot, dersKodu, egitmen) {
  const k = metin(dersKodu);
  if (!k) return false;
  const hocayaBak = egitmen !== undefined;
  return slotDersleri(slot).some(
    (d) => d.courseCode === k && (!hocayaBak || ayniEgitmen(d.instructor, egitmen))
  );
}
 ...
 */
export function sonrakiSube(slot, dersKodu, egitmen) {
  const k = metin(dersKodu);
  if (!k) return '1';
  const hocayaBak = egitmen !== undefined;
  const ayniKod = slotDersleri(slot).filter(
    (d) => d.courseCode === k && (!hocayaBak || ayniEgitmen(d.instructor, egitmen))
  );
...
function temizDers(d) {
  ...
  // Aynı gerekçeyle çakışma onayı da yalnız varsa yazılır. YAZMA yolu da
  // alanları tek tek seçtiği için burada taşınmazsa işaret hiç kaydedilmez
  // ve "bilerek kabul edildi" ayrımı ilk kaydetmede kaybolur.
  if (d.cakismaOnayi) out.cakismaOnayi = true;
  return out;
}
