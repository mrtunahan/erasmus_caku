...
export const PERFORMANS_BOLUMLERI = [
  {
    ...
    ikon: 'kalem',
    // ⚠ TAŞINDI: bu ekran artık akademisyenin "Benim Sayfam" modülünde bir
    // sekme. Akademisyen kendi verisini girmek için ayrı bir modüle gitmek
    // zorunda kalmasın diye oraya alındı. Tanım burada KALIYOR çünkü
    // `gorunumCoz` eski 'own' kimliğini hâlâ çözebilmeli (kayıtlı bağlantı
    // ya da eski durum 'own' taşıyor olabilir); yalnız artık bu modülün
    // gezinmesinde LİSTELENMİYOR.
    tasindi: true,
...
/** Veri girişi ekranının görünüm kimliği — gömülü çizim bunu kullanır. */
export const VERI_GIRISI_GORUNUMU = 'own';

/** Başka bir modüle taşınmış bölümler (bu modülün gezinmesinde çıkmaz). */
export function tasinmisMi(bolum) {
  return !!(bolum && bolum.tasindi);
}

/**
 * Kullanıcının görebildiği bölümler ve kapsamlar.
 * Hiç kapsamı kalmayan bölüm listeye girmez; taşınmış bölüm hiç girmez.
 */
export function kullanilabilirBolumler(yetkiler) {
  return PERFORMANS_BOLUMLERI.filter((b) => !tasinmisMi(b))
    .map((b) => ({
      ...b,
...
/**
 * Bu görünüm bu modülde kullanıcıya açık mı?
 * Taşınmış bölümün görünümü ARTIK AÇIK DEĞİLDİR: yetkisi olsa bile bu
 * modülde çizilmez, yoksa 'own' kayıtlı kalan kullanıcı gezinmede
 * karşılığı olmayan bir ekranda takılırdı.
 */
export function gorunumAcikMi(gorunum, yetkiler) {
  const c = gorunumCoz(gorunum);
  if (!c || tasinmisMi(c.bolum)) return false;
  return kapsamAcikMi(c.kapsam, yetkiler);
}

/**
 * Kullanıcının açılışta göreceği görünüm.
 *
 * Veri girişi taşındıktan sonra açılış, kullanıcının yetkisine göre ilk
 * AÇIK bölümdür. Hiç açık bölüm yoksa '' döner — eskiden burada 'own'
 * dönülüyordu; artık o ekran bu modülde olmadığı için 'own' döndürmek
 * çizilemeyen bir görünüm demek olurdu.
 */
