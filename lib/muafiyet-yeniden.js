...
export function sayaclar(matches) {
  ...
  ms.forEach((m) => {
    ...
    if (d === 'onayli') onayli += 1;
      ...
      // Sihirbaz taleplerinde `tier` satırın kendi kararıdır: otomatik muaf
      // (`approved`) ve otomatik red (`rejected`) satırlar akademisyenin
      // önüne düşmez, bekleyene sayılmaz. (Aynı kural:
      // lib/muafiyet-satir-yaz.js → sayaclar.)
      const tier = metin(m && m.tier).toLocaleLowerCase('tr');
