...
//   Fakülte yetkilisi: kendi fakültesindeki  Fakülte yetkilisi: YALNIZ kendi
//     bölümlerin anketleri + kendi fakülte      fakülte geneli anketleri
...
export function anketYonetilebilirMi(anket, kapsam, user) {
  ...
  if (!kapsamliMi(anket)) {
    const ad = metin(user && (user.name || user.identifier));
    return !!ad && metin(anket.createdBy) === ad;
  }
  ...
  if (k.kapsamTuru === 'fakulte') {
    // YALNIZ fakülte geneli ve KENDİ fakültesi. Bölüm anketine dokunulmaz.
    if (tur !== 'fakulte') return false;
    const anketFak = metin(anket.kapsamFacultyId);
    if (anketFak) return anketFak === metin(k.facultyId);
    // Fakülte kimliği yazılmamışsa bölüm listesinin kesişimine bakılır.
    return dizi(anket.kapsamDepartmentIds).some((x) => dizi(k.departmentIds).includes(x));
  }
...
export function anketKilitSebebi(anket, kapsam, user) {
  ...
  if (tur === 'fakulte') {
    return k.kapsamTuru === 'bolum'
      ? 'Fakülte geneli anket — düzenleme ve silme yetkisi fakülte yetkilisindedir.'
      : 'Başka bir fakültenin anketi.';
  }
...
export function kapsamOzetMetni(kapsam, ozet) {
  ...
  if (k.kapsamTuru === 'fakulte') {
    return (
      ...
      ' anket görünüyor · ' +
      o.yonetilebilir +
      ' tanesini düzenleyebilirsiniz (fakülte geneli anketleriniz). Bölüm anketleri yalnız görüntülenir.'
    );
  }
  return (
