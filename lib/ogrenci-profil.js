...
/**
 * KAYDEDİLECEK YAMA — silinen alan da yazılır.
 *
 * ⚠ BU FONKSİYON BİR HATADAN DOĞDU: öğrenci telefonunu/e-postasını siliyor,
 * kaydediyor, başka sekmeye gidip dönünce ESKİ DEĞER geri geliyordu.
 * Sebep: `profilNormalle` boş alanları çıktıya hiç koymaz, yazma ise
 * `merge: true` yapılır — eksik alan sunucuda "değişmedi" demektir,
 * "silindi" demez. Silinen değer bu yüzden kayda hiç ulaşmıyordu.
 *
 * Yama, SÖZLÜKTEKİ HER ALANI taşır: dolu olanlar değeriyle, boşaltılanlar
 * boş dizeyle. Bu yüzden yalnız TÜM formu gönderen çağrıda kullanılır
 * (Benim Sayfam'daki bilgi formu); kısmi bir güncellemede alanları silerdi.
 */
export function profilYazmaYamasi(ham) {
  const temiz = profilNormalle(ham);
  const cikti = {};
  PROFIL_ALANLARI.forEach((a) => {
    cikti[a.anahtar] = temiz[a.anahtar] || '';
  });
  return cikti;
}
