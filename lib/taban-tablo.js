...
// Sayı gibi görünen HERHANGİ bir belirteç — puan da olabilir başarı sırası da
// ("185.432", "1.234.567", "185432"). Sıra sütunu ayrı bir kalıpla
// TAHMİN EDİLMEZ: "185.432" hem 185 tam 432 küsurat bir puan hem de yüz
// seksen beş bin dört yüz otuz iki'lik bir sıra olabilir ve ikisini biçime
// bakarak ayırmak mümkün değil. Bu yüzden satırdaki tüm sayı sütunları
// saklanır, hangisinin taban sırası olduğunu panel KULLANICIYA sorar —
// tablonun hangi liste olduğunu sorduğu gibi.
 ...
 * @param {number} [siraSutunu=-1] taban BAŞARI SIRASI hangi sayı sütununda
 *   (-1: sıra okunmaz). Sütunlar `sayilar` dizisindeki sırayla numaralanır.
 * @returns {{ad:string, kod:string, taban:string, puanlar:string[],
 *   sayilar:string[], tabanSira:string, puanTuru:string, puansiz:boolean}|null}
 */
export function tabanSatiriCoz(ham, puanSutunu, siraSutunu) {
  ...
  // İlk puandan sonraki TÜM sayı sütunları — başarı sırası bunların biridir.
  // Ondan öncekiler (sıra no, kontenjan, yerleşen) ada ait bölgede kalır.
  const sayilar = parcalar.slice(ilkPuanIdx).filter((p) => SAYI_KALIBI.test(p));
  const siraIdx = Number.isFinite(Number(siraSutunu)) ? Number(siraSutunu) : -1;
  const tabanSira = siraIdx >= 0 && siraIdx < sayilar.length ? sayilar[siraIdx] : '';

  const { ad, puanTuru } = adCoz(parcalar, ilkPuanIdx, kod);
  if (!ad) return null;
  return { ad, kod, taban, puanlar, sayilar, tabanSira, puanTuru, puansiz: false };
}
 ...
 * @param {string} metin
 * @param {number} [puanSutunu=0] satırda birden çok puan varsa hangisi taban
 * @param {number} [siraSutunu=-1] taban başarı sırası hangi sayı sütununda
 */
export function tabanTablosuCoz(metin, puanSutunu, siraSutunu) {
  ...
  satirlar.forEach((s) => {
    ...
    toplamSatir += 1;
    const k = tabanSatiriCoz(satir, puanSutunu, siraSutunu);
    if (!k) {
  ...
  // Başarı sırası sütununu seçtirmek için: satırlarda kaç sayı sütunu var.
  const enCokSayiSutunu = kayitlar.reduce((m, k) => Math.max(m, (k.sayilar || []).length), 0);

  return { kayitlar, puansizlar, okunamayan, toplamSatir, enCokPuanSutunu, enCokSayiSutunu };
}
