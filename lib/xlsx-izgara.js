...
//
// ── DOLU, BOYALI VE BİRLEŞİK HÜCRELERE DOKUNULMAZ ──
...
// üçlüdür — hücrede metin varsa, hücre BOYALIYSA ya da hücre derslik
// sütunlarını kesen bir BİRLEŞİK ARALIĞIN içindeyse atlanır.
//
// Birleşik aralık ölçütü sonradan eklendi ve sebebi somuttur: kurumun kendi
// tablosunda öğle arası satırı (12:30 - 13:15) `E12:Q12` diye BİRLEŞTİRİLMİŞ
// ve BEYAZ bırakılmıştır — ne metni vardır ne boyası. Eski iki ölçüt bunu boş
// hücre sanıp içine ders yazıyordu; Excel birleşik aralıkta yalnız sol üst
// hücreyi gösterdiği için o ders ÇIKTIDA HİÇ GÖRÜNMÜYOR, üstelik "yazıldı"
// sayılıyordu. Sessizce kaybolan ders, eksik programdan da kötüdür.
// ══════════════════════════════════════════════════════════════
 ...
 * @returns {Object|null} { baslikSatiri, gunSutunu, saatSutunu, derslikler, satirlar, birlesikHucreler }
 *   derslikler:       [{ sutun, baslik, adaylar }]
 *   satirlar:         [{ satir, gun, saat }]
 *   birlesikHucreler: Set('E12', 'F12', …) — veri alanındaki birleşik
 *                     aralıkların KAPSADIĞI her hücre (sol üst dahil).
...
export function izgaraCoz(hucreler, birlesikler) {
  ...
  // Veri alanındaki birleşik aralıklar: derslik sütunlarını kesen her aralık
  // (öğle arası bandı gibi) tek bir hücre gibi davranır — içine ders yazılamaz.
  // Gün sütununun kendi blokları (C8:C17) buraya girmez: saat sütununun
  // solundadırlar ve zaten ızgaranın taşıyıcısıdırlar.
  const birlesikHucreler = new Set();
  (birlesikler || []).forEach((aralik) => {
    const [bas, son] = String(aralik).split(':');
    const b = refCoz(bas);
    const s = refCoz(son || bas);
    if (!b || !s) return;
    const ustSatir = Math.min(b.satir, s.satir);
    const altSatir = Math.max(b.satir, s.satir);
    if (altSatir <= baslikSatiri) return;
    const solIdx = Math.min(sutunIndeksi(b.sutun), sutunIndeksi(s.sutun));
    const sagIdx = Math.max(sutunIndeksi(b.sutun), sutunIndeksi(s.sutun));
    if (sagIdx <= saatIdx) return;
    for (let r = ustSatir; r <= altSatir; r++) {
      for (let c = Math.max(solIdx, saatIdx + 1); c <= sagIdx; c++) {
        birlesikHucreler.add(sutunAdi(c) + r);
      }
    }
  });

  return { baslikSatiri, gunSutunu, saatSutunu, derslikler, satirlar, birlesikHucreler };
}
...
export function yerlesimPlani(izgara, hucreler, kayitlar) {
  ...
  // derslik adayı → sütun
  //
  // Bir aday BİRDEN ÇOK sütunda geçiyorsa hiçbirine bağlanmaz. Başlıkların
  // parantez içi de aday sayıldığı için kapasite yazıları çakışıyor:
  // 'M10Z04 (T45 - S25)' ile 'M10Z06 (T45 - S25)' aynı 't45s25' adayını
  // üretir. "İlk gelen kazansın" demek, o adla eşleşen dersi SESSİZCE YANLIŞ
  // dersliğe yazmak olurdu; belirsiz aday düşürülür ve ders 'derslik-yok'
  // diye bildirilir — yetkili adı netleştirir.
  const adaySayaci = new Map();
  izgara.derslikler.forEach((d) => {
    new Set(d.adaylar).forEach((a) => adaySayaci.set(a, (adaySayaci.get(a) || 0) + 1));
  });
  const sutunDizini = new Map();
  izgara.derslikler.forEach((d) => {
    d.adaylar.forEach((a) => {
      if (adaySayaci.get(a) > 1) return;
  ...
  (kayitlar || []).forEach((k) => {
    ...
    const mevcut = h[ref] || {};
    // Birleşik aralık tek hücredir: Excel yalnız sol üstü gösterir, gerisine
    // yazılan değer belgede GÖRÜNMEZ. Beyaz bırakılmış öğle arası bandı bu
    // yüzden metinsiz ve boyasız olduğu hâlde yazılabilir alan değildir.
    if (izgara.birlesikHucreler && izgara.birlesikHucreler.has(ref)) {
      atlanan.push({ ...k, sebep: 'birlesik-hucre', ref });
      return;
    }
 ...
 *
 * Her sebep için OKUNMASI GEREKEN ŞEY farklıdır ve doğrusunu yazmak, sorunun
 * çözülüp çözülemeyeceğini belirler:
 *   • derslik sütunu yok → eşleşmeyen DERSLİK ADLARI. Ders kodunu yazmak işe
 *     yaramaz; yetkili şablondaki başlığı ya da derslik adını buna göre
 *     düzeltecektir.
 *   • gün/saat satırı yok → eksik olan GÜN + SAAT satırları. Ders kodunu
 *     saymak yetkiliyi şablonun neresine bakacağını bilmeden bırakıyordu:
 *     "MAK213 yazılamadı" cümlesi, şablona 'Cuma 17:15 - 18:00' satırının
 *     eklenmesi gerektiğini söylemez. Bu vaka gerçek çıktıda yaşandı —
 *     kurumun tablosunda 17:15 satırı YALNIZ Pazartesi'de var.
 *   • hücre engelli (dolu/boyalı/birleşik/kullanılmış) → hücrenin YERİ
 *     (gün · saat · derslik); yetkili şablonda o hücreye bakacaktır.
 *   • diğer sebepler     → DERS KODLARI.
 */
export function atlananOzeti(atlanan) {
  const sebepler = {
    ...
    'birlesik-hucre': 'şablonda o satır birleştirilmiş (öğle arası gibi ayrılmış bant)',
    'hucre-kullanildi': 'o hücreye başka bir ders yazıldı',
    'izgara-yok': 'şablonda ızgara bulunamadı',
  };
  const gruplar = new Map();
  (atlanan || []).forEach((a) => {
    const anahtar = a.sebep || '';
    if (!gruplar.has(anahtar)) gruplar.set(anahtar, []);
    gruplar.get(anahtar).push(a);
  });
  // Bir listeyi kısaltıp yazıya çevirir: uzun listeler uyarı kutusunu taşırır.
  const kisalt = (liste) =>
    liste.slice(0, 8).join(', ') + (liste.length > 8 ? ` … (+${liste.length - 8})` : '');
  const tekil = (liste) => [...new Set(liste.filter(Boolean))];
  const yer = (k) => [k.gun, k.saat].filter(Boolean).join(' ');

  return [...gruplar.entries()].map(([anahtar, kayitlar]) => {
    const sebep = sebepler[anahtar] || anahtar;
    const derslikBazli = anahtar === 'derslik-yok';
    const satirBazli = anahtar === 'saat-yok';
    const hucreBazli =
      anahtar === 'dolu-hucre' || anahtar === 'birlesik-hucre' || anahtar === 'hucre-kullanildi';
    const etiketler = tekil(kayitlar.map((k) => (derslikBazli ? k.derslik : k.kod) || '?'));
    const kodlar = kisalt(etiketler);
    // Sorunun ADRESİ: eksik satırlar ya da engelli hücreler.
    const nerede = satirBazli
      ? tekil(kayitlar.map(yer))
      : hucreBazli
        ? tekil(kayitlar.map((k) => [yer(k), k.derslik].filter(Boolean).join(' · ')))
        : [];
    let metin = `${kayitlar.length} ders yazılamadı (${sebep}): ${kodlar}`;
    if (derslikBazli) {
      metin = `${kayitlar.length} ders yazılamadı (${sebep}). Eşleşmeyen derslikler: ${kodlar}`;
    } else if (satirBazli && nerede.length) {
      metin =
        `${kayitlar.length} ders yazılamadı (${sebep}): ${kodlar}. ` +
        `Şablonda bulunmayan satırlar: ${kisalt(nerede)}`;
    } else if (hucreBazli && nerede.length) {
      metin = `${kayitlar.length} ders yazılamadı (${sebep}): ${kodlar}. Hücreler: ${kisalt(nerede)}`;
    }
    return { sebep, kod: anahtar, sayi: kayitlar.length, etiketler, nerede, metin };
  });
}

/** Şablonun derslik sütun başlıkları — "şablonda şunlar var" diyebilmek için. */
export function derslikBasliklari(izgara) {
  if (!izgara) return [];
  return izgara.derslikler.map((d) =>
    String(d.baslik || '')
      .replace(/\([^)]*\)/g, '')
      .replace(/[\r\n]+/g, ' ')
      .trim()
...
export function renkliStilEkle(stylesXml, istekler) {
  ...
  liste.forEach(({ temelStil, argb }) => {
    ...
    // applyFill="0" taşıyan biçime ikinci bir applyFill EKLENMEZ — çift
    // öznitelik XML'i bozar; mevcut değer 1'e çevrilir.
    if (/applyFill="/.test(yeni)) {
      yeni = yeni.replace(/applyFill="[^"]*"/, 'applyFill="1"');
    } else {
...
export function hucreleriYaz(sheetXml, yazimlar) {
  ...
  satirBazli.forEach((liste, satirNo) => {
    // Satır KENDİNİ KAPATAN biçimde de olabilir (<row r="9"/>): boş satırı
    // Excel böyle yazar. Açılış deseni onda '</row>' arar ve sonraki satırları
    // yutardı — kendini kapatan biçim ÖNCE denenir.
    const rx = new RegExp(
      '<row\\b[^>]*\\sr="' +
        satirNo +
        '"[^>]*/>|<row\\b[^>]*\\sr="' +
    ...
    if (!m) return;
    let satir = m[0];
    // Kendini kapatan satırı aç: içine hücre koyacağız.
    if (/\/>$/.test(satir) && !/<\/row>$/.test(satir)) {
      satir = satir.replace(/\/>$/, '></row>');
    }
    liste.forEach((y) => {
      ...
      const yeniHucre =
        ...
        kacisXml(y.deger) +
        '</t></is></c>';
      // ⚠ KENDİNİ KAPATAN HÜCRE TUZAĞI. Boş ama biçimli hücre '<c r="E9"
      // s="33"/>' yazılır. '[^>]*>…</c>' deseni bunda kapanışı bulamayınca
      // SONRAKİ hücrenin '</c>'sine kadar her şeyi yutuyordu; hedef satırın
      // son hücresindeyse '</row>' sınırını da aşıp XML'i bozuyordu — Excel
      // "dosyayı onarayım mı" diyordu. Tembel nicelik ('[^>]*?') iki biçimi
      // de kendi sınırında durdurur.
      const varRx = new RegExp('<c\\b[^>]*?\\sr="' + y.ref + '"[^>]*?(?:/>|>[\\s\\S]*?</c>)');
      if (varRx.test(satir)) {
 ...
 *
 * Bunlar KÜNYE ALANI DEĞİLDİR: ızgaranın işaretçisidir ve karşılıkları
 * sabittir. Şablonlar modülündeki eşleme sihirbazı '{{Ders Saati}}' adını
 * "Ders Saati Sayısı" değişkenine benzetip bağlayabildiği için, künye
 * değerleri bu iki başlığın ÜZERİNE YAZAMAZ (bkz. kunyeDoldur) — yoksa
 * tablonun sütun başlığı yerine bir sayı basılırdı.
 */
...
export function kunyeDoldur(sharedXml, degerler) {
  // Izgara işaretçileri en sonda: yanlış bir eşleme sütun başlığını bozamaz.
  const d = { ...(degerler || {}), ...IZGARA_BASLIKLARI };
