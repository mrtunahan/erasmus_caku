...
export function atlananOzeti(atlanan) {
  ...
  return [...gruplar.entries()].map(([anahtar, kayitlar]) => {
    ...
    const etiketler = [...new Set(kayitlar.map((k) => (derslikBazli ? k.derslik : k.kod) || '?'))];
    const gosterilen = etiketler.slice(0, 8).join(', ') + (etiketler.length > 8 ? ' …' : '');
    return {
      sebep,
      kod: anahtar,
      ...
      etiketler,
      metin: derslikBazli
        ? `${kayitlar.length} ders yazılamadı (${sebep}). Eşleşmeyen derslikler: ${gosterilen}`
        : `${kayitlar.length} ders yazılamadı (${sebep}): ${gosterilen}`,
    };
  });
}
