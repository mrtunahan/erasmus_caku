 ...
 *
 * GİDİŞ eşleştirmesi öğrenim anlaşmasının kendisidir: gitmeden önce
 * hazırlanır, imzalanır ve kurumlarca kabul edilir. Onu dönüşte bir kez daha
 * onaya düşürmek, çoktan imzalanmış bir belgeyi yeniden imzalatmaktır —
 * akademisyene her kayıtta aynı işi geri getiriyordu. Düzeltme gerekiyorsa
 * akademisyen zaten eşleştirmeyi doğrudan düzenleyebilir.
 *
 * Onaya tabi olan yalnız DÖNÜŞ tarafıdır: orada öğrenci ders değiştirmiş
 * olabilir ve notlar transkriptten hesaplanır.
 */
export function onayaTabiMi(tur) {
  return String(tur || '') === 'return';
}
...
/** Onay kuyruğuna giren eşleştirmeler: yalnız dönüş tarafı, yalnız 'pending'. */
export function onayBekleyenler(kayit) {
  const k = kayit || {};
  return (k.returnMatches || []).filter((m) => m && (m.status || 'approved') === 'pending');
}
