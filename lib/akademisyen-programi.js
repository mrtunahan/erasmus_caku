...
// Bir slotta kaç ders olduğu ve nasıl okunacağı TEK yerde: lib/ders-slot.js.
// Burada ayrı bir kopya vardı ve yalnız iki dersi biliyordu; hücreye üçüncü
// ders eklendiğinde akademisyenin programında sessizce eksik kalırdı.
import { slotDersleri } from './ders-slot.js';
import { akademisyenCakismalari, dersAraligi } from './seviye-cakisma.js';
// Saat etiketi artık BÖLÜME göre değişir (her bölüm programını istediği saatte
// başlatır — bkz. lib/ders-saatleri.js). Bu belge birden çok bölümü tek
// ızgarada gösterdiği için satırlar indekse değil SAATE göre dizilir.
import { VARSAYILAN_AYAR, etiketBaslangici, saatEtiketleri } from './ders-saatleri.js';

export const PROGRAM_GUNLERI = ['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma'];

// Bu belge birden çok bölümü birleştirdiği için GENEL bir saat ızgarasına
// ihtiyaç duyar. Izgara, saat ayarının varsayılanından türetilir — tek kaynak
// lib/ders-saatleri.js'tir, burada ikinci bir liste tutulmaz.
//
// Gün iki bloktur: sabah :30'da, öğleden sonra :15'te başlar (fakültenin kendi
// programı böyle). Lisansüstü akşam saatlerini de kapsar.
//
// ⚠ Slot anahtarı saat İNDEKSİDİR (`Pazartesi_3` gibi), etiket değil: bu
// dizinin SIRASI veri sözleşmesinin parçasıdır. Etiketleri değiştirmek kayıtlı
// programları bozmaz — satır eklemek/çıkarmak/sırasını değiştirmek bozar.
export const PROGRAM_SAATLERI = saatEtiketleri(VARSAYILAN_AYAR.lisansustu);
...
export function akademisyenKayitlari(dokumanlar, secenekler) {
  const { ad, donem, bolumler, bolumSaatleri } = secenekler || {};
  ...
  (dokumanlar || []).forEach((dok) => {
    ...
    Object.entries(slots).forEach(([anahtar, slot]) => {
      ...
      slotDersleri(slot).forEach((ders) => {
        ...
        gorulen.add(tekil);
        // ── ETİKET BÖLÜMÜN VE SEVİYENİN KENDİ LİSTESİNDEN GELİR ──
        // Saat ayarı bölüme VE seviyeye özeldir: lisans 08:30'da, lisansüstü
        // 18:00'de başlayabilir. Yalnız bölüme bakan bir arama, lisans
        // dersini lisansüstü saatiyle etiketler ve birleşik programda dersi
        // olmadığı saatte gösterir. Önce 'bolum|seviye', sonra eski
        // (yalnız bölüm) anahtarlar denenir — geriye uyumlu.
        const saatAra = (k) => bolumSaatleri && bolumSaatleri[k];
        const listem =
          saatAra(bolumId + '|' + seviye) ||
          saatAra(kanonBolum + '|' + seviye) ||
          saatAra(bolumId) ||
          saatAra(kanonBolum) ||
          PROGRAM_SAATLERI;
        kayitlar.push({
          gun,
          saatIndeksi,
          saat: listem[saatIndeksi] || PROGRAM_SAATLERI[saatIndeksi] || '',
  ...
  kayitlar.sort(
    (a, b) =>
      ...
      // İndeks DEĞİL saat: 08:30'da başlayan bölümün 0. saati, 08:15'te
      // başlayanın 0. saatiyle aynı zaman değildir.
      etiketBaslangici(a.saat) - etiketBaslangici(b.saat) ||
...
/**
 * Akademisyenin kendi programındaki zaman çakışmaları.
 *
 * Kayıtlar tek kişiye aittir; ayıran şey zamandır. `slotKimligi` hücreyi
 * temsil eder: aynı hücredeki iki ders (bölünmüş hücre) çakışma değildir,
 * aynı kodlu ders iki bölümde görünüyorsa da öyle — kural
 * `akademisyenCakismalari` içinde tek yerde durur.
 */
export function zamanCakismalari(kayitlar) {
  const girdiler = (kayitlar || [])
    .map((k) => {
      // Izgara satırıyla AYNI çözüm: etiket yoksa indeksten varsayılan
      // listeye düşülür, yoksa kayıt sessizce taramanın dışında kalırdı.
      const saat = (k && k.saat) || (k && PROGRAM_SAATLERI[k.saatIndeksi]) || '';
      const a = dersAraligi(saat);
      if (!a) return null;
      return {
        ...k,
        saat,
        ...a,
        akademisyen: 'x', // tek kişilik liste; ad ayırt edici değil
        hucre: k.slotKimligi || `${k.gun}|${k.saatIndeksi}`,
      };
    })
    .filter(Boolean);
  const etiket = (k) => `${k.dersKodu} (${k.bolumAdi})`;
  return akademisyenCakismalari(girdiler).map((c) => ({
    gun: c.gun,
    saat: (c.yeni && c.yeni.saat) || '',
    dersler: [etiket(c.yeni), etiket(c.mevcut)],
    mesaj:
      `${c.gun} ${c.yeni.saat} — ${c.yeni.dersKodu} / ${c.yeni.bolumAdi}` +
      ` & ${c.mevcut.dersKodu} / ${c.mevcut.bolumAdi}` +
      (c.mevcut.saat && c.mevcut.saat !== c.yeni.saat ? ` (${c.mevcut.saat})` : ''),
  }));
}
...
export function programIzgarasi(kayitlar) {
  ...
  // ── SATIR ANAHTARI SAAT ETİKETİDİR, İNDEKS DEĞİL ──
  // Bu belge birden çok bölümü tek tabloda birleştirir ve bölümler farklı
  // saatte başlayabilir. İndeksle anahtarlansaydı 08:15'teki ders 08:30'daki
  // dersle aynı satıra düşerdi.
  (kayitlar || []).forEach((k) => {
    if (!izgara[k.gun]) return;
    const saat = k.saat || PROGRAM_SAATLERI[k.saatIndeksi] || '';
    if (!izgara[k.gun][saat]) izgara[k.gun][saat] = [];
    izgara[k.gun][saat].push(k);
  });

  const gorulenSaat = new Set();
  (kayitlar || []).forEach((k) => {
    const saat = k.saat || PROGRAM_SAATLERI[k.saatIndeksi] || '';
    if (saat) gorulenSaat.add(saat);
  });
  const doluSaatler = [...gorulenSaat].sort(
    (a, b) => etiketBaslangici(a) - etiketBaslangici(b) || a.localeCompare(b, 'tr')
  );

  // ── ÇAKIŞMA HÜCREYE DEĞİL ZAMANA BAKAR ──
  // Tarama satır satır yapılıyordu: aynı saat ETİKETİNİ paylaşan kayıtlar.
  // Bu belge lisans ve lisansüstünü birlikte gösterir ve iki seviyenin saat
  // ayarı ayrıdır — biri 08:15'te, öteki 08:30'da başlayabilir. Etiketler
  // farklı olduğu için kayıtlar ayrı satırlara düşer ve gerçekten üst üste
  // binen iki ders çakışma sayılmazdı. Ölçüt artık etiketten çözülen zaman
  // aralığı (bkz. lib/seviye-cakisma.js); eşit etiket zaten kesişir, yani
  // eski durumlar da aynı sonucu verir.
  const cakismalar = zamanCakismalari(kayitlar);
...
export function akademisyenProgramHTML(kayitlar, secenekler) {
  ...
  const satirlar = doluSaatler
    .map((saat) => {
      ...
      return `<tr><td class="saat">${kacis(saat)}</td>${hucreler}</tr>`;
    })
    .join('');
