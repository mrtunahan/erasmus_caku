  ...
  (dokumanlar || []).forEach((dok) => {
    ...
    Object.entries(slots).forEach(([anahtar, slot]) => {
      ...
      slotDersleri(slot).forEach((ders) => {
        ...
        gorulen.add(tekil);
        // Etiket bölümün kendi saat listesinden gelir; ayarı olmayan bölüm
        // varsayılan ızgarayı kullandığı için liste bulunamazsa genel listeye
        // düşülür (davranış değişmez).
        const listem =
          (bolumSaatleri && (bolumSaatleri[bolumId] || bolumSaatleri[kanonBolum])) ||
          PROGRAM_SAATLERI;
        kayitlar.push({
          gun,
          saatIndeksi,
...
export function programIzgarasi(kayitlar) {
  ...
  const cakismalar = [];
  PROGRAM_GUNLERI.forEach((gun) => {
    Object.entries(izgara[gun]).forEach(([saat, liste]) => {
      if (!hucreCakisiyor(liste)) return;
      cakismalar.push({
        gun,
        saat,
        dersler: liste.map((k) => `${k.dersKodu} (${k.bolumAdi})`),
        mesaj: `${gun} ${saat} — ` + liste.map((k) => `${k.dersKodu} / ${k.bolumAdi}`).join(' & '),
      });
    });
  });
