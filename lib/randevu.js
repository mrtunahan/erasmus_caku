...
/** Slotu gerçekten DOLDURAN durumlar — yalnız onaylanmış randevu. */
...
/** Bu slot+tarih DOLU mu — yani onaylanmış bir randevusu var mı? */
export function slotMesgulMu(randevular, gun, saat, tarih) {
  return slottakiler(randevular, gun, saat, tarih, DOLU_DURUMLAR).length > 0;
}
 ...
 */
export function slotTalepleri(randevular, gun, saat, tarih, ogrenciNo) {
  const bekleyenler = slottakiler(randevular, gun, saat, tarih, ['bekliyor']);
  const no = metin(ogrenciNo);
  return {
    onayli: slotMesgulMu(randevular, gun, saat, tarih),
...
/**
 * Bir randevu onaylanınca AYNI SAATTE açıkta kalan diğer talepler.
 *
 * Bunlar sessizce bırakılamaz: saat artık dolu olduğu hâlde öğrencide
 * "bekliyor" yazmaya devam eder ve kimse sonucu öğrenemez. Çağıran bunları
 * reddeder (ya da hocaya sorar) — karar ekranın, liste burada.
 */
...
/** Slot rozetinin metni: 'Uygun' · '2 talep' · 'Dolu'. */
export function slotTalepMetni(durum) {
  const d = durum || {};
  if (d.onayli) return 'Dolu';
  if (d.benimTalebim) return 'Talebiniz var';
  if (d.bekleyen > 0) return d.bekleyen + ' talep';
  return 'Uygun';
}
...
export function engelMesaji(sebep) {
  switch (sebep) {
    ...
    case 'dolu':
      return 'Bu saat başka bir öğrenciye onaylandı. Başka bir saat seçin.';
    case 'bekleyen_var':
