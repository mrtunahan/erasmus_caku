...
const FormKarti = ({ form, kategori, isAdmin, onDelete, onDosyaDegistir }) => {
  ...
  const eskiFirebaseUrl =
    typeof form.dosyaURL === 'string' && form.dosyaURL.includes('firebasestorage.googleapis.com');
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        >
          <svg
            ...
            fill="none"
            viewBox="0 0 24 24"
            stroke={dosyaRenk(form.dosyaAdi)}
            strokeWidth={1.5}
          >
      ...
      >
        ...
        <span style={{ fontSize: 11, color: FM_C.textMuted }}>{formatTarih(form.createdAt)}</span>
      </div>
              ...
              <>
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M7 16a4 4 0 01-.88-7.9 5 5 0 019.9-1A5.5 5.5 0 0118.5 18H7zM12 12v6m-3-3l3-3 3 3"
                  />
                </svg>
...
const FormEkleModal = ({ onClose, onEkle, activeDepartment }) => {
  ...
  const handleSubmit = async () => {
    if (!baslik.trim()) {
      setHata('Form başlığı gereklidir.');
      return;
    }
    if (!dosya) {
      setHata('Lütfen bir dosya seçin.');
      return;
    }
...
function FormlarModuluApp({ currentUser, activeDepartment, departmentInfo }) {
  ...
  const dosyaDegistir = useCallback(async (form, file) => {
    ...
    if (eskiStoragePath && !eskiUrlFirebase) {
      try {
        await DB.deleteFormFile(eskiStoragePath);
      } catch (err) {
        console.warn('Eski dosya silinemedi:', err);
      }
    }

    setFormlar((prev) => prev.map((f) => (f.id === form.id ? { ...f, ...guncellemeVerisi } : f)));
  }, []);
  ...
  return (
    ...
    >
      ...
      {React.createElement(window.CakuBanner, {
        title: 'Formlar',
        subtitle: 'Gerekli form ve belgeleri buradan indirebilirsiniz',
      })}
      <div
        style={{
          display: 'flex',
        ...
        >
          <div
            style={{
              width: 24,
              height: 24,
              border: `3px solid ${FM_C.border}`,
              borderTopColor: FM_C.navy,
              borderRadius: '50%',
