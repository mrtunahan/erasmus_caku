...
const PasswordSecurity = {
  // SHA-256 hash üret (Web Crypto API with fallback)
  async sha256(message) {
    ...
    function sha256Fallback(msgBytes) {
      ...
      for (let off = 0; off < msgBytes.length; off += 64) {
        ...
        for (let i = 0; i < 16; i++) w[i] = (msgBytes[off+i*4]<<24)|(msgBytes[off+i*4+1]<<16)|(msgBytes[off+i*4+2]<<8)|msgBytes[off+i*4+3];
        for (let i = 16; i < 64; i++) {
