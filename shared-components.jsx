...
const LoginModal = ({ onLogin }) => {
  ...
  // Şifremi unuttum akışı — rol sekmeden gelir.
  ...
  return (
    <div className="lg-wrap">
      ...
      >
        ...
        {/* Şifremi unuttum — rol, açık olan giriş sekmesinden gelir. Admin
            hesabı için sıfırlama YOK: tek hesaptır ve kurtarma yolu
            /admin-reset üzerinden yöneticinin kendisindedir. */}
        {activeTab !== 'admin' && (
          <button
            type="button"
        ...
        {sifremiUnuttum && (
          <SifremiUnuttumModal
            rol={activeTab === 'student' ? 'student' : 'professor'}
            onKapat={() => setSifremiUnuttum(false)}
          />
        )}
...
// ŞİFREMİ UNUTTUM
//
// Üç adım: kimlik → kod → yeni şifre. Sunucu tarafı server/routes/auth.js
// (/forgot-password, /verify-reset-code, /reset-password).
//
// ── HESABIN VARLIĞI BURADA DA SIZDIRILMAZ ──
// Birinci adımın cevabı hesap olsa da olmasa da aynıdır; bu ekran da aynı
// mesajı gösterir. Kodun gittiği adres MASKELİ olarak bile yazılmaz —
// kullanıcı kendi adresini bilir, saldırgan kurbanınkini öğrenmemeli.
// ══════════════════════════════════════════════════════════════
function SifremiUnuttumModal({ rol, onKapat }) {
  const [adim, setAdim] = useState('kimlik');
  ...
  const cagir = async (yol, govde) => {
    ...
    setHata('');
    try {
      const r = await fetch('/api/auth/' + yol, {
        method: 'POST',
      ...
      return { error: 'Bağlantı hatası: ' + ((e && e.message) || '') };
    } finally {
      setBekle(false);
    }
  };
  ...
  const kodIste = async () => {
    if (!kimlik.trim()) return setHata('Kimlik bilgisi gerekli.');
    const d = await cagir('forgot-password', { role: rol, identifier: kimlik.trim() });
    if (d.error) return setHata(d.error);
    setBilgi(d.mesaj || 'Kod gönderildi.');
    setAdim('kod');
  };

  const kodDogrula = async () => {
    if (!kod.trim()) return setHata('Kodu girin.');
    const d = await cagir('verify-reset-code', {
      role: rol,
      identifier: kimlik.trim(),
      kod: kod.trim(),
    });
    if (!d.success) return setHata(d.error || 'Kod doğrulanamadı.');
    setJeton(d.sifirlamaJetonu);
    setAdim('sifre');
  };

  const sifreBelirle = async () => {
    if (yeni.length < 6) return setHata('Şifre en az 6 karakter olmalıdır.');
    if (yeni !== tekrar) return setHata('Şifreler eşleşmiyor.');
    const d = await cagir('reset-password', { sifirlamaJetonu: jeton, newPassword: yeni });
    if (!d.success) return setHata(d.error || 'Şifre belirlenemedi.');
    setAdim('bitti');
  };
  ...
  const dugme = {
    ...
    border: 'none',
    borderRadius: 8,
    background: bekle ? '#9CA3AF' : '#1B2A4A',
    color: '#fff',
    fontSize: 14,
    fontWeight: 700,
    fontFamily: 'inherit',
    cursor: bekle ? 'wait' : 'pointer',
  };
  const kimlikEtiketi =
    rol === 'student' ? 'Öğrenci numaranız' : 'Ad soyadınız (sistemde yazdığı gibi)';

  return (
    <div style={kutu} onClick={onKapat}>
      <div style={kart} onClick={(e) => e.stopPropagation()}>
        <div style={{ fontSize: 17, fontWeight: 700, color: '#1B2A4A', marginBottom: 4 }}>
          Şifremi unuttum
        </div>
        <div style={{ fontSize: 12.5, color: '#6B7280', marginBottom: 16, lineHeight: 1.5 }}>
          {adim === 'kimlik' && 'Kayıtlı e-posta adresinize tek kullanımlık bir kod göndereceğiz.'}
          {adim === 'kod' && bilgi}
          {adim === 'sifre' && 'Kod doğrulandı. Yeni şifrenizi belirleyin.'}
          {adim === 'bitti' && 'Şifreniz değiştirildi. Yeni şifrenizle giriş yapabilirsiniz.'}
        </div>
        ...
        {adim === 'kimlik' && (
          <>
            <label style={{ fontSize: 13, fontWeight: 600 }}>{kimlikEtiketi}</label>
            <input
              value={kimlik}
              onChange={(e) => setKimlik(e.target.value)}
              style={{ ...giris, marginTop: 6 }}
              autoFocus
            />
            <button onClick={kodIste} disabled={bekle} style={dugme}>
        ...
        {adim === 'kod' && (
          <>
            <label style={{ fontSize: 13, fontWeight: 600 }}>E-postanıza gelen kod</label>
            <input
              value={kod}
              onChange={(e) => setKod(e.target.value)}
              inputMode="numeric"
              placeholder="6 haneli kod"
              style={{ ...giris, marginTop: 6, letterSpacing: 4, fontSize: 18 }}
              autoFocus
            />
            <button onClick={kodDogrula} disabled={bekle} style={dugme}>
              {bekle ? 'Doğrulanıyor…' : 'Kodu doğrula'}
            </button>
          </>
        )}
        ...
        {adim === 'sifre' && (
          <>
            <label style={{ fontSize: 13, fontWeight: 600 }}>Yeni şifre</label>
            <input
              type="password"
              value={yeni}
              ...
              autoFocus
            />
            <label style={{ fontSize: 13, fontWeight: 600 }}>Yeni şifre (tekrar)</label>
            <input
              type="password"
              value={tekrar}
              ...
              style={{ ...giris, marginTop: 6 }}
            />
            <button onClick={sifreBelirle} disabled={bekle} style={dugme}>
              {bekle ? 'Kaydediliyor…' : 'Şifreyi belirle'}
            </button>
          </>
        )}
        ...
        >
          {adim === 'bitti' ? 'Giriş ekranına dön' : 'Vazgeç'}
        </button>
      </div>
    </div>
  );
}
window.SifremiUnuttumModal = SifremiUnuttumModal;
