...
const AL = {
  primary: "#0891B2", primaryPale: "#ECFEFF",
  navy: "#1E293B", text: "#334155", textMuted: "#64748B",
  border: "#E5E7EB",
};
...
function AuditLogModuluApp({ currentUser, activeDepartment }) {
  ...
  const targetOptions = useMemo(() => {
    const s = new Set();
    logs.forEach(l => l.target && s.add(l.target));
    return Array.from(s).sort();
  }, [logs]);
  ...
  const filtered = useMemo(() => {
    return logs.filter(l => {
      ...
      if (filterTarget !== "all" && l.target !== filterTarget) return false;
      if (search) {
        const s = search.toLowerCase();
        const hay = `${l.actor || ""} ${l.action || ""} ${l.target || ""} ${l.targetId || ""}`.toLowerCase();
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
        <input
          ...
          style={{
            ...
            borderRadius: 8, border: `1px solid ${AL.border}`, fontSize: 13, outline: "none",
          }}
        />
        <select value={filterAction} onChange={e => setFilterAction(e.target.value)}
          style={{ padding: "9px 12px", borderRadius: 8, border: `1px solid ${AL.border}`, fontSize: 13, background: "white", cursor: "pointer" }}>
      ...
      {/* Table */}
      <div style={{ background: "white", borderRadius: 12, border: `1px solid ${AL.border}`, overflow: "auto" }}>
        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: AL.textMuted }}>Yükleniyor...</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: AL.textMuted }}>Kayıt bulunamadı.</div>
        ) : (
            ...
            <thead>
              <tr style={{ background: "#FAFAFA", borderBottom: `2px solid ${AL.border}` }}>
                {["Tarih", "Aktör", "Rol", "Aksiyon", "Hedef", "Kayıt ID", "Detay"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "10px 12px", fontWeight: 700, color: AL.navy, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, 500).map((l, i) => (
                <tr key={l.id || i} style={{ borderBottom: "1px solid #F3F4F6" }}>
                  ...
                  <td style={{ padding: "8px 12px", fontWeight: 600, color: AL.text }}>{l.actor || "—"}</td>
                  <td style={{ padding: "8px 12px", color: AL.textMuted }}>
                    {l.actorRole === "admin" ? "Admin"
                      : l.actorRole === "bolum_yetkilisi" ? "Bölüm Yetk."
                      : l.actorRole === "professor" ? "Akademisyen"
                      : l.actorRole || "—"}
                  </td>
                  <td style={{ padding: "8px 12px" }}>
                    <span style={{
                      padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                      background: AL.primaryPale, color: AL.primary,
                    }}>{l.action}</span>
                  </td>
                  <td style={{ padding: "8px 12px", color: AL.text }}>{l.target || "—"}</td>
                  <td style={{ padding: "8px 12px", fontFamily: "monospace", fontSize: 11, color: AL.textMuted }}>{l.targetId || "—"}</td>
                  <td style={{ padding: "8px 12px", fontSize: 11, color: AL.textMuted, maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                      title={(() => { try { return JSON.stringify(l.meta || {}); } catch { return ""; } })()}>
        ...
        {filtered.length > 500 && (
          <div style={{ padding: 12, textAlign: "center", color: AL.textMuted, fontSize: 11, borderTop: `1px solid ${AL.border}` }}>
