...
function color(c, s) {
  const codes = { reset: "\x1b[0m", red: "\x1b[31m", green: "\x1b[32m", yellow: "\x1b[33m", blue: "\x1b[34m", grey: "\x1b[90m" };
...
async function run() {
  ...
  console.log(color("grey", `  Toplam: ${files.length} dosya, uygulanmış: ${applied.size}, bekleyen: ${pending.length}`));
  ...
  if (flags.status || flags.dry) {
    ...
    if (flags.status) { await disconnect(); return; }
  }
  ...
  for (const file of pending) {
    ...
    if (flags.dry) { console.log(color("grey", "  (dry-run: çalıştırılmadı)")); continue; }
