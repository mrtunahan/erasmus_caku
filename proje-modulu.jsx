...
var ProjDB = {
  ...
  _col: function (category) {
    var cat = PROJECT_CATEGORIES.find(function (c) {
...
function exportProjectsXLSX(projects, courseName) {
  ...
  projects.forEach(function (p) {
    if (p.members && p.members.length > maxMembers) maxMembers = p.members.length;
  });
  ...
  files.forEach(function (f) {
    ...
    var localHeader = new Uint8Array([
      ...
      0x00,
      ...intToLE(crc, 4),
      ...intToLE(dataBytes.length, 4),
      ...intToLE(dataBytes.length, 4),
      ...intToLE(nameBytes.length, 2),
      0x00,
      0x00,
    ]);
  ...
  var cdSize = centralDir.reduce(function (s, a) {
    return s + a.length;
  }, 0);
  ...
  var blob = new Blob([].concat(parts, centralDir, [eocd]), {
...
function exportProjectsWord(projects, courseName) {
  ...
  var html =
    ...
    '<div class="info"><b>Ders:</b> ' +
    (courseName || 'Tüm Dersler') +
    ' &nbsp;&nbsp;|&nbsp;&nbsp; <b>Toplam Grup:</b> ' +
    projects.length +
    ' &nbsp;&nbsp;|&nbsp;&nbsp; <b>Toplam Katılımcı:</b> ' +
    projects.reduce(function (s, p) {
      return s + (p.members ? p.members.length : 0);
    }, 0) +
    ' &nbsp;&nbsp;|&nbsp;&nbsp; <b>Tarih:</b> ' +
...
function ProjectCard({
  ...
  var myInviteStatus =
    myMemberIdx >= 0 && memberStatuses[myMemberIdx] ? memberStatuses[myMemberIdx] : null;
          ...
          >
            ...
            {project.scheduleDate && project.scheduleTime && (
              ...
              >
                ...
                Sunum:{' '}
                {new Date(project.scheduleDate).toLocaleDateString('tr-TR', {
                  day: 'numeric',
                  month: 'long',
                })}{' '}
          ...
          {(isAdmin || isOwner) && (
            <button
              ...
              onMouseEnter={function (e) {
                e.currentTarget.style.color = PRJ.red;
                e.currentTarget.style.background = 'rgba(220,38,38,0.08)';
              }}
              onMouseLeave={function (e) {
                e.currentTarget.style.color = PRJ.textMuted;
      ...
      {expanded && (
        ...
        >
          ...
          >
            ...
            Olusturan:{' '}
            <span style={{ fontWeight: 600, color: PRJ.text }}>{project.createdByName}</span>
          </div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, color: '#92400e', fontWeight: 600 }}>
                  Bu gruba davet edildiniz
                </span>
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <button
                  onClick={function () {
                    onRespondInvite(project.id, myMemberIdx, 'accepted');
                  }}
                  style={{
                    background: 'linear-gradient(135deg, #059669, #10b981)',
                    color: 'white',
                ...
                <button
                  onClick={function () {
                    onRespondInvite(project.id, myMemberIdx, 'rejected');
                  }}
                  style={{
                    background: 'linear-gradient(135deg, #dc2626, #ef4444)',
                    color: 'white',
          ...
          {project.summary && (
            ...
            >
              <p style={{ fontSize: 13.5, color: PRJ.text, lineHeight: 1.65, margin: 0 }}>
                {project.summary}
              </p>
            </div>
          )}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background:
                        'rgba(' +
                        (idx === 0 ? '99,102,241' : idx === 1 ? '5,150,105' : '234,88,12') +
                        ',0.04)',
                      border:
                        '1px solid rgba(' +
                        (idx === 0 ? '99,102,241' : idx === 1 ? '5,150,105' : '234,88,12') +
...
function CreateProjectModal({ onClose, onCreate, currentUserName, minGroupSize, maxGroupSize }) {
  ...
  var removeMember = function (idx) {
    if (members.length <= minSize || idx === 0) return;
    setMembers(function (p) {
  ...
  var updateMember = function (idx, v) {
    setMembers(function (p) {
  ...
  var handleSubmit = function () {
    ...
    var valid = members.filter(function (m) {
      return m.trim();
    });
    if (valid.length < minSize) {
      alert('En az ' + minSize + ' kişi olmalıdır!');
      return;
    }
    if (valid.length > maxSize) {
      alert('En fazla ' + maxSize + ' kişi olabilir!');
      return;
    }
    var unique = [];
    valid.forEach(function (m) {
      if (unique.indexOf(m.trim().toLowerCase()) < 0) unique.push(m.trim().toLowerCase());
    });
    ...
    >
      <div
        style={{
          background: 'white',
          borderRadius: 16,
          padding: window.innerWidth <= 480 ? 16 : 32,
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 13, color: PRJ.primary, margin: 0, lineHeight: 1.5 }}>
            ...
            {minSize === maxSize
              ? minSize + ' kişiden'
              : minSize + ' ile ' + maxSize + ' kişi arasından'}{' '}
            oluşabilir. İlk üye olarak siz otomatik eklenirsiniz.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          >
            Grup Üyeleri * (
            {minSize === maxSize ? minSize + ' kişi' : minSize + '-' + maxSize + ' kişi'})
          </label>
              ...
              >
                <div
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    color: idx === 0 ? 'white' : PRJ.textMuted,
                    fontSize: 12,
                    fontWeight: 700,
                    flexShrink: 0,
                  }}
...
function AddCourseModal({
  ...
  var cs = useState(editCourse ? editCourse.code : ''),
    code = cs[0],
    setCode = cs[1];
  var ns = useState(editCourse ? editCourse.name : ''),
    name = ns[0],
    setName = ns[1];
  var ps = useState(editCourse ? editCourse.professor || '' : ''),
    prof = ps[0],
    setProf = ps[1];
  var ds = useState(editCourse ? editCourse.deadline || '' : ''),
    deadline = ds[0],
    setDeadline = ds[1];
  var pds = useState(editCourse ? editCourse.projectPeriod || '' : ''),
    projectPeriod = pds[0],
    setProjectPeriod = pds[1];
  var mns = useState(editCourse ? editCourse.minGroupSize || 2 : 2),
    minGroupSize = mns[0],
    setMinGroupSize = mns[1];
  var mxs = useState(editCourse ? editCourse.maxGroupSize || 3 : 3),
    maxGroupSize = mxs[0],
    setMaxGroupSize = mxs[1];
  var scs = useState(''),
  ...
  var handleSubmit = function () {
    ...
    if (minG < 1) {
      alert('Minimum grup boyutu en az 1 olmalıdır!');
      return;
    }
    if (maxG < minG) {
      alert('Maksimum grup boyutu, minimum grup boyutundan küçük olamaz!');
      return;
    }
    if (maxG > 10) {
      alert("Maksimum grup boyutu 10'u geçemez!");
      return;
    }
    onAdd({
      code: code.trim(),
    ...
    >
      <div
        style={{
          background: 'white',
          borderRadius: 16,
          padding: window.innerWidth <= 480 ? 16 : 28,
      ...
      >
        ...
        >
          <PrjIcon path={PRJ_ICONS.book} size={20} color={PRJ.primary} />{' '}
          {editCourse
            ? 'Düzenle'
            : categoryLabel
              ? categoryLabel + ' - Yeni Proje Alanı Ekle'
              : 'Yeni Ders Ekle'}
        </h3>
        ...
        {!editCourse && isProfessor && uniqueCourses.length === 0 && (
          ...
          >
            <b>Size tanımlı ders bulunamadı.</b>
            <br />
            ...
            {availableCourses && availableCourses.length > 0 && (
              <div style={{ marginTop: 6, fontSize: 11, color: '#78350F' }}>
                (Bölümünüzde {availableCourses.length} ders var ama hiçbiri "{currentUserName}"
                adıyla eşleşmiyor. Ders Yönetimi'ndeki "Akademisyen" alanı tam adınızla aynı
                olmalı.)
              </div>
            )}
          </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            ...
            style={{
              ...
              outline: 'none',
              fontFamily: "'Source Sans 3', sans-serif",
              background: isProfessor && !!selectedCourseId ? '#F3F4F6' : 'white',
            }}
          />
        </div>
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            ...
            placeholder="Bilgisayar Projesi I"
            readOnly={isProfessor && !!selectedCourseId}
            style={{
              width: '100%',
              padding: '10px 14px',
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input
            ...
            placeholder="2025-2026 Güz / 2025-2026 Bahar"
            style={{
              width: '100%',
              padding: '10px 14px',
              border: '1px solid ' + PRJ.border,
        ...
        <p style={{ fontSize: 11, color: PRJ.textMuted, marginTop: -8, marginBottom: 14 }}>
          Öğrenciler bu aralıkta grup oluşturabilir. (Varsayılan: 2-3 kişi)
        </p>
        <div style={{ marginBottom: 20 }}>
          <label
            style={{
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <PrjIcon path={PRJ_ICONS.calendar} size={14} color={PRJ.primary} /> Proje Son Tarihi
              (Deadline)
            </span>
          </label>
          <input
          ...
          />
          <p style={{ fontSize: 11, color: PRJ.textMuted, marginTop: 4, marginBottom: 0 }}>
            Bu tarihten sonra yeni proje grubu oluşturulamaz. Boş bırakılırsa sınır yoktur.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button
...
function BulkScheduleModal({ onClose, onDistribute, projectCount }) {
  var _s = useState;
  var ss = _s([{ date: '', time: '14:00 - 15:00', capacity: 5 }]),
    sessions = ss[0],
    setSessions = ss[1];

  var addSession = function () {
    setSessions(function (p) {
      return p.concat([{ date: '', time: '14:00 - 15:00', capacity: 5 }]);
    });
  };
  ...
  var totalCapacity = sessions.reduce(function (acc, s) {
    return acc + (parseInt(s.capacity) || 0);
  }, 0);

  var handleSubmit = function () {
    var valid = sessions.filter(function (s) {
      return s.date && s.time && s.capacity > 0;
    });
    if (valid.length === 0) {
      alert('Lütfen en az bir geçerli oturum ekleyin.');
      return;
    }
    if (totalCapacity < projectCount) {
      if (
        !confirm(
          'Oturum toplam kapasitesi (' +
            totalCapacity +
            ') onaylı/bekleyen proje sayısından (' +
            projectCount +
            ') az. Yine de devam edilsin mi? (Açıkta kalan projeler planlanmayacak.)'
        )
      ) {
        return;
      }
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: 13, color: PRJ.textMuted, marginBottom: 20, lineHeight: 1.5 }}>
          Bekleyen/Onaylanmış <b>{projectCount} adet proje</b> için sunum ve değerlendirme tarihleri
          atayın. Oturumları açtıktan sonra "Dağıt ve Kaydet" butonuna bastığınızda seçili projeler
          belirtilen oturumlara dağıtılacaktır. (Not: Tamamen rastgele değil, sırayla atanır.)
        </p>
              ...
              >
                ...
                <div style={{ flex: '1 1 70px' }}>
                  ...
                  <input
                    ...
                    value={s.capacity}
                    onChange={function (e) {
                      updateSession(idx, 'capacity', parseInt(e.target.value));
                    }}
                    style={{
                      width: '100%',
                      padding: '8px 12px',
        ...
        >
          ...
          <div
            style={{
              fontSize: 13,
              fontWeight: 600,
              color: totalCapacity < projectCount ? PRJ.orange : PRJ.green,
              background: totalCapacity < projectCount ? PRJ.orangeLight : PRJ.greenLight,
              padding: '6px 12px',
              borderRadius: 8,
            }}
          >
...
const PROJECT_CATEGORIES = [
  {
    id: 'bolum',
    label: 'Bölüm',
    icon: PRJ_ICONS.users,
    color: '#3B82F6',
    description: 'Bölüm içi ders bazlı proje grupları',
    collection: 'project_courses',
  },
];
...
function ProjeModuluApp({ currentUser, activeDepartment, departmentInfo }) {
  ...
  var vs = _s(null),
    selectedCourse = vs[0],
    setSelectedCourse = vs[1]; // null = ders listesi, object = ders detayı
  var sms = _s(false),
    showCreateModal = sms[0],
    setShowCreateModal = sms[1];
  var scm = _s(false),
    showCourseModal = scm[0],
    setShowCourseModal = scm[1];
  var sbms = _s(false),
    showBulkScheduleModal = sbms[0],
    setShowBulkScheduleModal = sbms[1];
  var sq = _s(''),
    searchQuery = sq[0],
    setSearchQuery = sq[1];
  var ecv = _s(null),
    editingCourse = ecv[0],
    setEditingCourse = ecv[1]; // ders düzenleme
  var dcs = _s([]),
    deptCourses = dcs[0],
  ...
  var aps = _s([]),
    allProjects = aps[0],
    setAllProjects = aps[1];
  var sccs = _s(null),
    studentCourseCodes = sccs[0],
    setStudentCourseCodes = sccs[1]; // öğrencinin seçtiği ders kodları
  ...
  useEffect(
    function () {
      ...
      window
        .apiRead('sinav_dersler', { where: where })
        .then(function (all) {
          // apiRead bazen tüm bölümleri dönebildiğinden güvenlik için tekrar filtrele
          var filtered = (all || []).filter(function (c) {
            return c.departmentId === activeDepartment;
          });
          setDeptCourses(filtered);
        })
        .catch(function (err) {
          console.error('Bölüm dersleri yüklenemedi:', err);
        });
    },
    [activeDepartment]
  );
  ...
  // birleştirilir. Geriye dönük: eski students.myCourseIds de eklenir.
  useEffect(
    function () {
      ...
      window
        ...
        .then(function (docs) {
          ...
          // Legacy fallback: eski tek-liste seçim
          return window.DB.fetchStudents();
        })
        .then(function (students) {
          var me = (students || []).find(function (s) {
  ...
  var loadAllProjects = useCallback(
    function () {
      ProjDB.fetchAllProjects(activeCategory)
        .then(function (data) {
          setAllProjects(data);
        })
        .catch(function () {});
  ...
  useEffect(
    function () {
      ...
      setProjects([]);
      loadAllProjects();
      ProjDB.fetchCourses(activeCategory, activeDepartment)
        .then(function (data) {
          var filtered;
          if (isProfessor && userName) {
            filtered = data.filter(function (c) {
              return prjMatchesProfessor(c.professor, userName);
            });
  ...
  useEffect(
    function () {
      ...
      setLoading(true);
      ProjDB.fetchProjects(selectedCourse.id, activeCategory)
        .then(function (data) {
          setProjects(data);
          setLoading(false);
        })
      ...
      if (editingCourse) {
        ...
        setCourses(function (prev) {
          return prev.map(function (c) {
            return c.id === editingCourse.id ? Object.assign({}, c, courseData) : c;
          });
        });
        if (selectedCourse && selectedCourse.id === editingCourse.id) {
          setSelectedCourse(function (prev) {
            return Object.assign({}, prev, courseData);
          });
        }
        ...
        setCourses(function (prev) {
          return prev.concat([Object.assign({}, courseData, { id: ref.id })]);
        });
        ...
        try {
          ...
          console.warn('Bildirim gönderilemedi:', ne);
        }
      }
    ...
    for (var i = 0; i < projects.length; i++) {
      ...
      if (
        ...
        p.members.some(function (m) {
          return m.trim().toLowerCase() === nameLower;
        })
      ) {
        return p;
      }
  ...
  var userExistingProject = useMemo(
    function () {
    ...
    try {
      ...
      setProjects(function (prev) {
        return prev.map(function (p) {
          return p.id === projectId ? Object.assign({}, p, { status: 'approved' }) : p;
        });
      });
  ...
  var handleRejectProject = async function (projectId) {
    ...
    try {
      ...
      setProjects(function (prev) {
        return prev.map(function (p) {
          return p.id === projectId ? Object.assign({}, p, { status: 'rejected' }) : p;
        });
      });
      setAllProjects(function (prev) {
        return prev.map(function (p) {
  ...
  var handleDistributeSchedule = async function (sessions) {
    var targets = projects.filter(function (p) {
      return !p.status || p.status === 'approved' || p.status === 'pending';
    });
    if (targets.length === 0) {
      alert('Dağıtım yapılacak onaylı veya bekleyen proje yok.');
      return;
    }
    ...
    try {
      ...
      for (var i = 0; i < targets.length; i++) {
        ...
        if (
          timeParts.length === 2 &&
          timeParts[0].indexOf(':') !== -1 &&
          timeParts[1].indexOf(':') !== -1
        ) {
        ...
        await ProjDB.updateProject(
          p.id,
          { scheduleDate: s.date, scheduleTime: slotTime },
          activeCategory
        );
      ...
      alert(
        assignedCount +
          ' proje başarıyla planlandı. ' +
          (targets.length - assignedCount > 0
            ? targets.length - assignedCount + ' proje kapasite yetersizliğinden açıkta kaldı.'
            : 'Tüm onaylı/bekleyen projeler kapandı.')
      );
    } catch (e) {
      setLoading(false);
    ...
    try {
      ...
      if (response === 'accepted') {
        var courseInfo =
          ...
          '\nDers: ' +
          (project.courseName || 'Bilinmeyen Ders');
        // courseCode alanı varsa kullan, yoksa courseName'den çıkar (format: "BIL401 - Ders Adı")
        var projectCourseCode =
          project.courseCode ||
          (project.courseName ? project.courseName.split(' - ')[0].trim() : '');
      ...
      var updater = function (prev) {
        return prev.map(function (p) {
          return p.id === projectId ? Object.assign({}, p, { memberStatus: newStatuses }) : p;
        });
      };
      setProjects(updater);
  ...
  var handleCreateProject = async function (data) {
    ...
    for (var i = 0; i < projects.length; i++) {
      var existingMembers = (projects[i].members || []).map(function (m) {
        return m.trim().toLowerCase();
      });
        ...
        if (newMembers[j] && existingMembers.indexOf(newMembers[j]) >= 0) {
          alert(
            '"' +
              ...
              '" bu derste zaten "' +
              projects[i].name +
              '" projesinde yer alıyor. Aynı derste birden fazla proje grubunda yer alamazsınız!'
          );
          return;
        }
      }
    ...
    try {
      ...
      if (selectedCourse.deadline) {
        ...
        if (now > dl) {
          alert(
            'Bu ders için proje grubu oluşturma süresi dolmuştur! (Son tarih: ' +
      ...
      for (var ci = 0; ci < freshCourseProjects.length; ci++) {
        var p = freshCourseProjects[ci];
        if (
          p.members &&
          p.members.some(function (m) {
            return m.trim().toLowerCase() === creatorNameLower;
          })
        ) {
          creatorExisting = p;
          break;
      ...
      if (creatorExisting) {
        alert(
          'Bu derste zaten bir proje grubundasınız: "' +
            creatorExisting.name +
            '". Aynı derste birden fazla proje grubunda yer alamazsınız.'
        );
        return;
      }

      // Kontrol: Eklenen diğer üyeler aynı derste zaten bir projede mi?
      for (var cj = 0; cj < data.members.length; cj++) {
        var mName = data.members[cj];
        ...
        for (var k = 0; k < freshCourseProjects.length; k++) {
          ...
          if (
            pp.members &&
            pp.members.some(function (m) {
              return m.trim().toLowerCase() === mNameLower;
            })
          ) {
            memberExisting = pp;
            break;
      ...
      var mStatuses = data.members.map(function (m, idx) {
        return idx === 0 ? 'accepted' : 'pending';
      });
  ...
  if (!selectedCourse) {
    var activeCat =
      PROJECT_CATEGORIES.find(function (c) {
    ...
    return (
      <div style={{ background: PRJ.bg, minHeight: '100vh', padding: '0 0 40px' }}>
        ...
        >
          ...
          {/* Ders Kartları */}
          {loading ? (
            <div style={{ textAlign: 'center', padding: 60, color: PRJ.textMuted }}>
              Yükleniyor...
            </div>
                ...
                return (
                  <div
                    ...
                    onMouseEnter={function (e) {
                      ...
                      e.currentTarget.style.borderColor = PRJ.primary;
                      e.currentTarget.style.transform = 'translateY(-2px)';
                    }}
                    onMouseLeave={function (e) {
                      e.currentTarget.style.boxShadow = 'none';
                      e.currentTarget.style.borderColor = PRJ.border;
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 16, fontWeight: 600, color: PRJ.text }}>
                          {course.name}
                        </div>
                      </div>
                      {isAdmin && (
                      ...
                      (function () {
                        ...
                        return (
                          <div
                            style={{
                              fontSize: 12,
                              color: passed ? PRJ.red : PRJ.orange,
                              display: 'flex',
                              alignItems: 'center',
                              gap: 4,
                              marginTop: 4,
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder="Proje veya kisi ara..."
              style={{
                flex: 1,
                border: 'none',
                outline: 'none',
          ...
          {isAdmin &&
            (function () {
              var pendingCount = projects.filter(function (p) {
        ...
        {isAdmin &&
          (function () {
            ...
            Object.keys(memberMap).forEach(function (key) {
              if (memberMap[key].length > 1)
                duplicates.push({ name: key, projects: memberMap[key] });
            });
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <strong style={{ fontSize: 14, color: PRJ.red }}>
                    Birden fazla projede yer alan öğrenciler ({duplicates.length} kişi)
                  </strong>
                </div>
                {duplicates.map(function (d, i) {
                  return (
                    <div
                      key={i}
                      style={{ fontSize: 13, color: '#991b1b', marginBottom: 4, paddingLeft: 26 }}
                    >
